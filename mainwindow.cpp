...
namespace
{
  ...
  {
    return type >= 0 && (((type < 6 || 7 == type) && (msg_parts.contains("73") || msg_parts.contains("RR73"))) || (type == 6 && !msg_parts.filter("73").isEmpty()));
  }
...
MainWindow::MainWindow(bool multiple, QSettings *settings, QSharedMemory *shdmem,
                       ...
                       QProcessEnvironment const &env, QWidget *parent) : QMainWindow(parent),
                                                                          m_exitCode{0},
...
//		else { ui->stopTxButton->click(); }
  ...
  connect(ui->band_hopping_schedule_push_button, &QPushButton::clicked, &m_WSPR_band_hopping, &WSPRBandHopping::show_dialog);
  connect(ui->sbTxPercent, static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged), &m_WSPR_band_hopping, &WSPRBandHopping::set_tx_percent);
  ...
  auto band_validator = new LiveFrequencyValidator{ui->bandComboBox, m_config.bands(), m_config.frequencies(), &m_freqNominal, this};
  ...
  connect(ui->bandComboBox->lineEdit(), &QLineEdit::textEdited, [this](QString const &)
          {m_bandEdited = true; if(m_config.write_decoded_debug()) writeToALLTXT("bandComboBox line edited to " + ui->bandComboBox->lineEdit()->text()); });
  ...
  connect(ui->tx5->lineEdit(), &QLineEdit::editingFinished, [this]()
          { on_tx5_currentTextChanged(ui->tx5->lineEdit()->text()); });
  ui->freeTextMsg->setModel(m_config.macros());
  connect(ui->freeTextMsg->lineEdit(), &QLineEdit::editingFinished, [this]()
          { on_freeTextMsg_currentTextChanged(ui->freeTextMsg->lineEdit()->text()); });
  ...
  if (!m_multiple)
  {
    while (true)
    {
      ...
      if (iret == 603)
        break;
      JTDXMessageBox::warning_message(this, "", tr("Error Killing jtdxjt9.exe Process"), tr("KillByName return code: %1").arg(iret));
    }
  }
#endif
...

  {
    ...
    while (quitFile.exists())
    {
      if (!quitFile.remove())
      {
        JTDXMessageBox::query_message(this, "", tr("Error removing \"%1\"").arg(quitFile.fileName()), tr("Click OK to retry"));
      }
    }
  }
  ...
  QStringList jt9_args{
      ...
      ,
      "-e", QDir::toNativeSeparators(m_appDir), "-a", QDir::toNativeSeparators(m_dataDir.absolutePath()), "-t", QDir::toNativeSeparators(m_config.temp_dir().absolutePath()), "-r", QDir::toNativeSeparators(m_config.data_dir().absolutePath())};
  ...
  if (m_config.monitor_off_at_startup())
    m_monitoroff = true;
  ...
  if (m_mode == "FT8")
    ...
    on_actionT10_triggered();
  else if (m_mode == "WSPR-2")
    on_actionWSPR_2_triggered();

  if (m_mode != "FT8")
  {
    ui->actionEnable_hound_mode->setChecked(false);
    ui->actionEnable_hound_mode->setEnabled(false);
  }
  ...
  if (ui->actionEnable_hound_mode->isChecked())
    on_actionEnable_hound_mode_toggled(true);
  ...
  if (m_rprtPriority && m_maxDistance)
  {
    ui->actionMaxDistance->setChecked(false);
    m_maxDistance = false;
  }
  ...
  if (!ui->cbShowWanted->isChecked())
  {
    ui->cbShowWanted->setChecked(true);
    ui->cbShowWanted->setChecked(false);
  }
  m_oldmode = m_mode;
  ...
  if (!m_config.my_callsign().isEmpty())
    toggle_skipTx1();
  ui->spotMsgLabel->setVisible(false);
  ui->spotEditLabel->setVisible(false);
  ui->spotLineEdit->setVisible(false);
  ui->propEditLabel->setVisible(false);
  ui->propLineEdit->setVisible(false);
  ...
  foxgen_();
  ui->actionLatvian->setEnabled(false);
  ui->actionDutch->setEnabled(false); // temporarily disable

  // make sure TX hash tables are filled in at first message transmission
  char message[38] = "CQ QQ1QQQ KO85                       ";
  char msgsent[38];
  char ft8msgbits[77];
  int i3 = 0;
  int n3 = 0;
...
void MainWindow::writeSettings()
{
  ...
  m_settings->setValue("TxAfterQSO", m_tx_after_qso);
...
void MainWindow::readSettings()
{
  ...
  if (m_rrr)
  {
    ui->pbSendRRR->setText("RRR");
  }
  else
  {
    ui->pbSendRRR->setText("RR73");
  }

  m_cqdir = m_settings->value("CQdirection", "").toString();
  if (!m_cqdir.isEmpty())
  {
    QRegularExpression cqdir_re("^[A-Za-z]{2,2}$");
    QRegularExpressionMatch match = cqdir_re.match(m_cqdir);
  ...
  if (!m_cqdir.isEmpty())
  {
    ui->pbCallCQ->setText("CQ " + m_cqdir);
  }
  else
  {
    ui->pbCallCQ->setText("CQ");
  }
  ...
  if (m_settings->value("ShowMenus").toString() == "false")
  {
    ui->cbMenus->setChecked(false);
    on_cbMenus_toggled(false);
  }
  else
  {
    ui->cbMenus->setChecked(true);
    on_cbMenus_toggled(true);
  }

  bool wanted = m_settings->value("ShowWanted", false).toBool();
  m_wantedchkd = wanted;
  ui->cbShowWanted->setChecked(wanted);
  ...
  if (!(m_ft8threads >= 0 && m_ft8threads < 25))
    m_ft8threads = 0;
  if (m_ft8threads == 0)
    ui->actionMTAuto->setChecked(true);
  else if (m_ft8threads == 1)
    ui->actionMT1->setChecked(true);
  else if (m_ft8threads == 2)
    ui->actionMT2->setChecked(true);
  else if (m_ft8threads == 3)
    ui->actionMT3->setChecked(true);
  else if (m_ft8threads == 4)
    ui->actionMT4->setChecked(true);
  else if (m_ft8threads == 5)
    ui->actionMT5->setChecked(true);
  else if (m_ft8threads == 6)
    ui->actionMT6->setChecked(true);
  else if (m_ft8threads == 7)
    ui->actionMT7->setChecked(true);
  else if (m_ft8threads == 8)
    ui->actionMT8->setChecked(true);
  else if (m_ft8threads == 9)
    ui->actionMT9->setChecked(true);
  else if (m_ft8threads == 10)
    ui->actionMT10->setChecked(true);
  else if (m_ft8threads == 11)
    ui->actionMT11->setChecked(true);
  else if (m_ft8threads == 12)
    ui->actionMT12->setChecked(true);
  else if (m_ft8threads == 13)
    ui->actionMT13->setChecked(true);
  else if (m_ft8threads == 14)
    ui->actionMT14->setChecked(true);
  else if (m_ft8threads == 15)
    ui->actionMT15->setChecked(true);
  else if (m_ft8threads == 16)
    ui->actionMT16->setChecked(true);
  else if (m_ft8threads == 17)
    ui->actionMT17->setChecked(true);
  else if (m_ft8threads == 18)
    ui->actionMT18->setChecked(true);
  else if (m_ft8threads == 19)
    ui->actionMT19->setChecked(true);
  else if (m_ft8threads == 20)
    ui->actionMT20->setChecked(true);
  else if (m_ft8threads == 21)
    ui->actionMT21->setChecked(true);
  else if (m_ft8threads == 22)
    ui->actionMT22->setChecked(true);
  else if (m_ft8threads == 23)
    ui->actionMT23->setChecked(true);
  else if (m_ft8threads == 24)
    ui->actionMT24->setChecked(true);

  m_acceptUDP = m_settings->value("AcceptUDPReplyMessages", 1).toInt();
  if (!(m_acceptUDP >= 1 && m_acceptUDP <= 3))
    m_acceptUDP = 1;
  if (m_acceptUDP == 1)
    ui->actionAcceptUDPCQ->setChecked(true);
  else if (m_acceptUDP == 2)
    ui->actionAcceptUDPCQ73->setChecked(true);
  else if (m_acceptUDP == 3)
    ui->actionAcceptUDPAny->setChecked(true);
  ...
  if (!m_modeTx.startsWith("FT") && !m_modeTx.startsWith("JT") && m_modeTx != "T10" && !m_modeTx.startsWith("WSPR"))
  {
    if (m_mode == "FT8")
      ...
      m_modeTx = "T10";
    else if (m_modeTx.startsWith("WSPR"))
      m_modeTx = "WSPR-2";
  }
  if (m_modeTx.startsWith("JT9"))
    ui->pbTxMode->setText("Tx JT9  @");
  if (m_modeTx == "JT65")
    ui->pbTxMode->setText("Tx JT65  #");

  m_saveWav = m_settings->value("SaveWav", 0).toInt();
  if (!(m_saveWav >= 0 && m_saveWav <= 2))
    m_saveWav = 0;
  if (m_saveWav == 0)
  ...
  if (!(m_callMode >= 0 && m_callMode <= 3))
    m_callMode = 2;
  if (m_callMode == 0)
    ui->actionCallNone->setChecked(true);
  else if (m_callMode == 1)
    ui->actionCallFirst->setChecked(true);
  else if (m_callMode == 2)
    ui->actionCallMid->setChecked(true);
  else if (m_callMode == 3)
    ui->actionCallEnd->setChecked(true);
  ...
  if (m_settings->value("CallPriorityCQ").toString() != "false" && m_settings->value("CallPriorityCQ").toString() != "true")
    ...
    ui->actionCallPriorityAndSearchCQ->setChecked(m_settings->value("CallPriorityCQ", false).toBool());
  ...
  if (m_settings->value("HideFTContestMessages").toString() != "false" && m_settings->value("HideFTContestMessages").toString() != "true")
    ...
    ui->actionHide_FT_contest_messages->setChecked(m_settings->value("HideFTContestMessages").toBool());
  ...
  if (m_settings->value("HideTelemetryMessages").toString() != "false" && m_settings->value("HideTelemetryMessages").toString() != "true")
    ...
    ui->actionHide_telemetry_messages->setChecked(m_settings->value("HideTelemetryMessages").toBool());
  ...
  if (!(m_ft8Sensitivity >= 0 && m_ft8Sensitivity <= 2))
    m_ft8Sensitivity = 0;
  if (m_ft8Sensitivity == 0)
    ui->actionFT8SensMin->setChecked(true);
  else if (m_ft8Sensitivity == 1)
    ui->actionlowFT8thresholds->setChecked(true);
  else if (m_ft8Sensitivity == 2)
    ui->actionFT8subpass->setChecked(true);
  ...
  if (m_settings->value("FT8DecoderEarlyStart").toString() != "false" && m_settings->value("FT8DecoderEarlyStart").toString() != "true")
    ...
    ui->actionFT8EarlyStart->setChecked(m_settings->value("FT8DecoderEarlyStart", false).toBool());
  ...
  if (m_settings->value("NCandidateListThinning").toInt() >= 5 && m_settings->value("NCandidateListThinning").toInt() <= 100)
    ...
    ui->candListSpinBox->setValue(100);

  ui->DTCenterSpinBox->setValue(0.1); // ensure a change is signaled
  qint32 nDTcenter = m_settings->value("DTCenterInt", 0).toInt();
  if (nDTcenter > -201 && nDTcenter < 251)
    ui->DTCenterSpinBox->setValue(static_cast<double>(nDTcenter / 100.));
  else
    ui->DTCenterSpinBox->setValue(0.0);

  m_nFT8Cycles = m_settings->value("NFT8Cycles", 1).toInt();
  if (!(m_nFT8Cycles >= 1 && m_nFT8Cycles <= 3))
    m_nFT8Cycles = 1;
  if (m_nFT8Cycles == 1)
    ui->actionDecFT8cycles1->setChecked(true);
  else if (m_nFT8Cycles == 2)
    ui->actionDecFT8cycles2->setChecked(true);
  else if (m_nFT8Cycles == 3)
    ui->actionDecFT8cycles3->setChecked(true);

  m_nFT8SWLCycles = m_settings->value("NFT8SWLCycles", 1).toInt();
  if (!(m_nFT8SWLCycles >= 1 && m_nFT8SWLCycles <= 3))
    m_nFT8SWLCycles = 1;
  if (m_nFT8SWLCycles == 1)
    ui->actionDecFT8SWLcycles1->setChecked(true);
  else if (m_nFT8SWLCycles == 2)
    ui->actionDecFT8SWLcycles2->setChecked(true);
  else if (m_nFT8SWLCycles == 3)
    ui->actionDecFT8SWLcycles3->setChecked(true);

  m_nFT8RXfSens = m_settings->value("NFT8QSORXfreqSensitivity", 2).toInt();
  if (!(m_nFT8RXfSens >= 1 && m_nFT8RXfSens <= 3))
    m_nFT8RXfSens = 1;
  if (m_nFT8RXfSens == 1)
    ui->actionRXfLow->setChecked(true);
  else if (m_nFT8RXfSens == 2)
    ui->actionRXfMedium->setChecked(true);
  else if (m_nFT8RXfSens == 3)
    ui->actionRXfHigh->setChecked(true);

  m_nFT4depth = m_settings->value("NFT4Depth", 3).toInt();
  if (!(m_nFT4depth >= 1 && m_nFT4depth <= 3))
    m_nFT4depth = 3;
  if (m_nFT4depth == 3)
    ui->actionFT4deep->setChecked(true);
  else if (m_nFT4depth == 2)
    ui->actionFT4medium->setChecked(true);
  else if (m_nFT4depth == 1)
    ui->actionFT4fast->setChecked(true);

  m_FilterState = m_settings->value("SwitchFilterOff", 0).toInt();
  if (!(m_FilterState >= 0 && m_FilterState <= 2))
    m_FilterState = 0;
  if (m_FilterState == 0)
    on_actionAutoFilter_toggled(false);
  else if (m_FilterState == 1)
  {
    ui->actionSwitch_Filter_OFF_at_sending_73->setChecked(true);
    on_actionSwitch_Filter_OFF_at_sending_73_triggered(true);
  }
  else if (m_FilterState == 2)
  {
    ui->actionSwitch_Filter_OFF_at_getting_73->setChecked(true);
    on_actionSwitch_Filter_OFF_at_getting_73_triggered(true);
  }
  ...
  if (m_settings->value("WSPRfreq").toInt() >= 1400 && m_settings->value("WSPRfreq").toInt() <= 1600)
    ...
    ui->WSPRfreqSpinBox->setValue(1500);
  ...
  m_lastMonitoredFrequency = m_settings->value("DialFreq",
                                               QVariant::fromValue<Frequency>(default_frequency))
                                 .value<Frequency>();
  ...
  if (!(m_pctx >= 0 && m_pctx <= 100))
    m_pctx = 20;
  m_dBm = m_settings->value("dBm", 37).toInt();
  if (!(m_dBm >= -30 && m_dBm <= 60))
    m_dBm = 37;
  ...
  m_tx_after_qso = m_settings->value("TxAfterQSO", false).toBool();
  ui->actionTxAfterQSO->setChecked(m_tx_after_qso);
  ...
  if (m_config.do_snr())
  {
    if (ui->S_meter_button->isChecked())
      ui->S_meter_button->setText("S0");
    else
      ui->S_meter_button->setText("dBm");
  }
  else
    ui->S_meter_button->setText(tr("S meter"));
...
void MainWindow::setStopHSym()
{
  ...
  if (m_mode == "FT8")
  {
    ...
    m_hsymStop = 21;
  else if (m_mode.startsWith("JT") or m_mode == "T10")
  {
    m_hsymStop = 173;
  ...
  if (m_start || reset)
  {
    if (m_mode.startsWith("FT"))
    {
      if (m_mode == "FT8")
      {
        ...
        if ((isecond >= 0 && isecond < 15) || (isecond >= 30 && isecond < 45))
      ...
      {
        if (ft4int == 0 || ft4int == 2 || ft4int == 4 || ft4int == 6)
          ui->labUTC->setStyleSheet(QString("font-size: 18pt;background: %1;color : %2").arg(Radio::convert_dark("#96ffff", m_useDarkStyle), Radio::convert_dark("#1400b1", m_useDarkStyle)));
        else
          ui->labUTC->setStyleSheet(QString("font-size: 18pt;background: %1;color: %2").arg(Radio::convert_dark("#ffff96", m_useDarkStyle), Radio::convert_dark("#0000ff", m_useDarkStyle)));
      }
...
void MainWindow::setAutoSeqButtonStyle(bool checked)
{
  if (checked)
  {
    if (m_singleshot)
    {
      ui->AutoSeqButton->setStyleSheet(QString("QPushButton {color: %1;background: %2;border-style: solid;border-width: 1px;border-radius: 5px;border-color: %3;min-width: 5em;padding: 3px}").arg(Radio::convert_dark("#0000ff", m_useDarkStyle), Radio::convert_dark("#00ff00", m_useDarkStyle), Radio::convert_dark("#000000", m_useDarkStyle)));
    }
    else
    {
      ui->AutoSeqButton->setStyleSheet(QString("QPushButton {color: %1;background: %2;border-style: solid;border-width: 1px;border-radius: 5px;border-color: %3;min-width: 5em;padding: 3px}").arg(Radio::convert_dark("#000000", m_useDarkStyle), Radio::convert_dark("#00ff00", m_useDarkStyle), Radio::convert_dark("#000000", m_useDarkStyle)));
    }
  }
  else
  {
    if (m_singleshot)
    {
      if (m_mode.startsWith("FT"))
      {
        ui->AutoSeqButton->setStyleSheet(QString("QPushButton {color: %1;background: %2;border-style: solid;border-width: 1px;border-color: %3;min-width: 5em;padding: 3px}").arg(Radio::convert_dark("#0000ff", m_useDarkStyle), Radio::convert_dark("#ffbbbb", m_useDarkStyle), Radio::convert_dark("#808080", m_useDarkStyle)));
      }
      else
      {
        ui->AutoSeqButton->setStyleSheet(QString("QPushButton {color: %1;background: %2;border-style: solid;border-width: 1px;border-color: %3;min-width: 5em;padding: 3px}").arg(Radio::convert_dark("#0000ff", m_useDarkStyle), Radio::convert_dark("#e0e0e0", m_useDarkStyle), Radio::convert_dark("#808080", m_useDarkStyle)));
      }
    }
    else
    {
      if (m_mode.startsWith("FT"))
      {
        ui->AutoSeqButton->setStyleSheet(QString("QPushButton {color: %1;background: %2;border-style: solid;border-width: 1px;border-color: %3;min-width: 5em;padding: 3px}").arg(Radio::convert_dark("#000000", m_useDarkStyle), Radio::convert_dark("#ffbbbb", m_useDarkStyle), Radio::convert_dark("#808080", m_useDarkStyle)));
      }
      else
      {
        ui->AutoSeqButton->setStyleSheet(QString("QPushButton {color: %1;background: %2;border-style: solid;border-width: 1px;border-color: %3;min-width: 5em;padding: 3px}").arg(Radio::convert_dark("#000000", m_useDarkStyle), Radio::convert_dark("#e0e0e0", m_useDarkStyle), Radio::convert_dark("#808080", m_useDarkStyle)));
      }
    }
  }
}
...
void MainWindow::setMinButton()
{
  if (!m_mode.startsWith("WSPR"))
  {
    if (m_txFirst)
    {
      ...
      {
        if (m_mode == "FT8")
          ui->TxMinuteButton->setText("TX 00/30");
        else
          ui->TxMinuteButton->setText("TX 00");
      }
      else
        ui->TxMinuteButton->setText(tr("TX Even"));
    ...
    else
    {
      ...
      {
        if (m_mode == "FT8")
          ui->TxMinuteButton->setText("TX 15/45");
        else
          ui->TxMinuteButton->setText("TX 7.5");
      }
      else
        ui->TxMinuteButton->setText(tr("TX Odd"));
...
void MainWindow::autoStopTx(QString reason)
{
  ...
  {
    if (!(m_tx_after_qso && reason.contains("end of QSO") && !m_is_manual_tx))
    {
      haltTx(reason);
    }
    else
...
void MainWindow::writeHaltTxEvent(QString reason)
{
  ...
  if (m_config.write_decoded_debug())
  {
    ...
    {
      JTDXMessageBox::warning_message(this, "", tr("File Open Error"), tr("Cannot open \"%1\" for append: %2").arg(f.fileName()).arg(f.errorString()));
    }
  }
}
...
void MainWindow::dataSink(qint64 frames)
{
  ...
  if (m_diskData || m_mode.startsWith("WSPR"))
    m_delay = 0;
  ...
  if (!m_diskData && m_mode == "FT8")
  {
    ...
    if (nlostaudio > 0)
    {
      ...
      {
        if (nlostaudio < 3)
          ui->label_6->setStyleSheet(QString("QLabel{background: %1}").arg(Radio::convert_dark("#ffff00", m_useDarkStyle)));
        else
          ui->label_6->setStyleSheet(QString("QLabel{background: %1}").arg(Radio::convert_dark("#ff8000", m_useDarkStyle)));
        ui->label_6->setText(tr("lost audio ") + QString::number(nlostaudio));
        if (m_config.write_decoded_debug())
          writeToALLTXT("Lost audio blocks: " + QString::number(nlostaudio));
      }
      else
  ...
  if (m_delay > 0)
  {
    ...
    if (fmod(fdelta, 1.0) > 0.49)
      ihsymdelay = qCeil(fdelta) + ihsym;
    else
      ihsymdelay = qFloor(fdelta) + ihsym;
  }
  // cycling approximately once per 269..301 milliseconds
  if ((m_mode == "FT8" && m_delay == 0 && ihsym == nhsymEStopFT8) || (m_mode == "FT8" && m_delay > 0 && ihsymdelay >= nhsymEStopFT8) || (m_mode == "FT4" && m_delay == 0 && ihsym == m_hsymStop) || (m_mode == "FT4" && m_delay > 0 && ihsymdelay >= m_hsymStop) || ((m_mode.startsWith("JT") || m_mode == "T10") && m_delay == 0 && ihsym == m_hsymStop) || ((m_mode.startsWith("JT") || m_mode == "T10") && m_delay > 0 && ihsymdelay >= m_hsymStop) || (m_mode.startsWith("WSPR") && ihsym == m_hsymStop))
  {
    QDateTime now = m_jtdxtime->currentDateTimeUtc2();
    // prevent dupe decoding
    if (lastdelayed && !m_modeChanged)
    {
      if (m_mode == "FT8" && last.secsTo(now) < 12)
      {
        lastdelayed = false;
        return;
      }
      else if (m_mode == "FT4" && last.secsTo(now) < 6)
      {
        lastdelayed = false;
        return;
      }
      else if (!m_mode.startsWith("FT") && !m_mode.startsWith("WSPR") && last.secsTo(now) < 46)
      {
        lastdelayed = false;
        return;
      }
    ...
    if (!m_decoderBusy && m_mode.left(4) != "WSPR")
    { // Start decoder
      last = now;
      decode();
      if (!m_lostaudio)
    ...
    if (!m_diskData && (m_saveWav != 0 || m_mode.mid(0, 4) == "WSPR"))
    { // Save unless "Save None"
      if (m_mode.startsWith("FT"))
      {
      ...
      if (m_mode.mid(0, 4) == "WSPR")
      {
        ...
        if (err != 0)
          JTDXMessageBox::warning_message(this, "", tr("Error saving c2 file"), c2name);
      }
    }
    ...
    if (m_mode.left(4) == "WSPR" && !m_decoderBusy)
    {
      ...
      if (m_ndepth == 1)
        depth_string = " -qB "; // 2 pass w subtract, no Block detection, no shift jittering
      if (m_ndepth == 2)
        depth_string = " -B "; // 2 pass w subtract, no Block detection
      if (m_ndepth == 3)
        depth_string = " -C 5000 -o 4"; // 2 pass w subtract, Block detection and OSD.
                                   ...
                                   Frequency frequency, QString const &his_call, QString const &his_grid, JTDXDateTime *jtdxtime) const
{
  ...
  BWFFile::InfoDictionary list_info{
      ...
      {{{'I', 'C', 'R', 'D'}}, jtdxtime->currentDateTime2().toString("yyyy-MM-ddTHH:mm:ss.zzzZ").toLocal8Bit()},
      {{{'I', 'C', 'M', 'T'}}, comment.toLocal8Bit()},
  };
  BWFFile wav{format, name + ".wav", list_info};
  if (!wav.open(BWFFile::WriteOnly) || 0 > wav.write(reinterpret_cast<char const *>(data), sizeof(short) * samples))
  {
...
void MainWindow::on_actionSettings_triggered() // Setup Dialog
{
  ...
  {
    if (m_config.write_decoded_debug())
      writeToALLTXT("Configuration settings change accepted");
    ui->decodedTextBrowser->setConfiguration(&m_config);
    ui->decodedTextBrowser2->setConfiguration(&m_config);
    ...
    if (m_config.my_callsign() != m_callsign)
    {
      ...
      if (!m_config.my_callsign().isEmpty())
      {
        if (m_bMyCallStd)
        {
          if (!ui->skipTx1->isEnabled())
          {
            ui->skipTx1->setEnabled(true);
            ui->skipGrid->setEnabled(true);
          }
        }
        else
        {
          ...
          {
            m_skipTx1 = false;
            ui->skipTx1->setChecked(false);
            ui->skipGrid->setChecked(false);
          }
          ui->skipTx1->setEnabled(false);
          ui->skipGrid->setEnabled(false);
        }
      }
      m_baseCall = Radio::base_callsign(m_config.my_callsign());
      ui->genStdMsgsPushButton->click();
      ...
      setLastLogdLabel();
      morse_(const_cast<char *>(m_config.my_callsign().toLatin1().constData()), const_cast<int *>(icw), &m_ncw, m_config.my_callsign().length());
    }
    ...
    if (m_monitoring && (m_config.restart_tci() || m_tci != (m_config.tci_audio() && m_config.is_tci())))
    ...
    if (m_mode != "WSPR-2" && spot_to_dxsummit != m_config.spot_to_dxsummit())
    {
      ...
      {
        ui->pbSpotDXCall->setStyleSheet(QString("QPushButton {color: %1;background: %2;border-style: outset;border-width: 1px;border-color: %3;padding: 3px}").arg(Radio::convert_dark("#000000", m_useDarkStyle), Radio::convert_dark("#c4c4ff", m_useDarkStyle), Radio::convert_dark("#808080", m_useDarkStyle)));
      }
      else
      {
        ui->pbSpotDXCall->setStyleSheet(QString("QPushButton {color: %1;background: %2;border-style: outset;border-width: 1px;border-color: %3;padding: 3px}").arg(Radio::convert_dark("#000000", m_useDarkStyle), Radio::convert_dark("#aabec8", m_useDarkStyle), Radio::convert_dark("#808080", m_useDarkStyle)));
      }
    }
    if (!m_config.do_snr())
      ui->S_meter_button->setText(tr("S meter"));
...
void MainWindow::on_AutoTxButton_clicked(bool checked)
{
  m_autoTx = checked;
  if (checked)
    ui->AutoTxButton->setStyleSheet(QString("QPushButton {color: %1;background: %2;border-style: solid;border-width: 1px;border-radius: 5px;border-color: %3;min-width: 5em;padding: 3px}").arg(Radio::convert_dark("#000000", m_useDarkStyle), Radio::convert_dark("#00ff00", m_useDarkStyle), Radio::convert_dark("#000000", m_useDarkStyle)));
  else if (m_autoseq)
    ui->AutoTxButton->setStyleSheet(QString("QPushButton {color: %1;background: %2;border-style: solid;border-width: 1px;border-color: %3;min-width: 5em;padding: 3px}").arg(Radio::convert_dark("#000000", m_useDarkStyle), Radio::convert_dark("#ffbbbb", m_useDarkStyle), Radio::convert_dark("#808080", m_useDarkStyle)));
  else
    ui->AutoTxButton->setStyleSheet(QString("QPushButton {color: %1;background: %2;border-style: solid;border-width: 1px;border-color: %3;min-width: 5em;padding: 3px}").arg(Radio::convert_dark("#000000", m_useDarkStyle), Radio::convert_dark("#e0e0e0", m_useDarkStyle), Radio::convert_dark("#808080", m_useDarkStyle)));
}
  ...
  if (state)
  {
    ...
    if (!m_monitoring)
    {
      ...
      if (m_addtx == -1)
        ...
        m_addtx = 0; // no delay for manual triggering Monitor button
      if (m_mode == "FT8")
      {
        curdsec = curdsec % 150;
        if (curdsec > 0 && curdsec < 90)
          m_delay = curdsec + m_addtx;
        else
          m_delay = 0;
      }
      else if (m_mode == "FT4")
      ...
      {
        if (curdsec > 0 && curdsec < 350)
          m_delay = curdsec + m_addtx;
        else
          m_delay = 0; // 2 second processing delay
      }
      m_addtx = 0;
    }
  }
...
{
  if (m_enableTx && !checked && m_curMsgTx.startsWith(m_hisCall + " "))
    m_lasthint = true;
  if (checked && m_lasthint)
    m_lasthint = false;
  m_enableTx = checked;
  statusUpdate();
...
void MainWindow::keyPressEvent(QKeyEvent *e) // keyPressEvent
{
  ...
  case Qt::Key_F7:
    if (!m_menus)
      on_actionOpen_wsjtx_log_adi_triggered();
    return;
    break;
  case Qt::Key_F11:
    n = 11;
  ...
  case Qt::Key_F:
    ...
    if (e->modifiers() & Qt::AltModifier)
    {
      if (m_bypassAllFilters)
      {
        ui->actionBypass_all_text_filters->setChecked(false);
        m_bypassAllFilters = false;
      }
      else
      {
        ui->actionBypass_all_text_filters->setChecked(true);
        m_bypassAllFilters = true;
      }
    }
    break;
  case Qt::Key_G:
...
void MainWindow::displayDialFrequency()
{
  ...
  Frequency dial_frequency{m_rigState.ptt() && m_rigState.split() ? m_rigState.tx_frequency() : m_rigState.frequency()};
  if (m_monitoroff && m_config.rig_name() == "None")
    dial_frequency = m_freqNominal;
  // lookup band
  ...
  if (m_lastBand != band_name && ui->bandComboBox->currentText() != band_name)
  {
    ...
    if (!m_lastBand.isEmpty() && !band_name.isEmpty())
    { // handle loss of the poll response from transceiver where dial_frequency==0
      if (m_autoEraseBC && !startup)
      { // option: erase both windows if band is changed
  ...
  for (auto const &item : *m_config.frequencies())
  {
    ...
    auto const &offset = dial_frequency > working_frequency ? dial_frequency - working_frequency : working_frequency - dial_frequency;
  ...
  if (valid)
    ui->labDialFreq->setStyleSheet(QString("QLabel {font-family: MS Shell Dlg 2;font-size: 18pt;background: %1;color: %2;}").arg(Radio::convert_dark("#e1e1e1", m_useDarkStyle), Radio::convert_dark("#0000ff", m_useDarkStyle)));
  else
    ui->labDialFreq->setStyleSheet(QString("QLabel {font-family: MS Shell Dlg 2;font-size: 18pt;background: %1;color: %2;}").arg(Radio::convert_dark("#ff0000", m_useDarkStyle), Radio::convert_dark("#0000ff", m_useDarkStyle)));
  ...
  if (((first_freq && dial_frequency != 0 && dial_frequency != 145000000) || (first_value != m_lastDialFreq)) && m_mode == "FT8")
  {
    ...
    if (m_houndMode)
    {
      ...
      else
      {
        ...
        if (m_commonFT8b || m_config.rig_name() == "None")
          ui->actionUse_TX_frequency_jumps->setEnabled(false);
        else
...
void MainWindow::styleChanged()
{
  ...
  if (!m_uploadSpots)
    ui->cbUploadWSPR_Spots->setStyleSheet(QString("QCheckBox{background: %1}").arg(Radio::convert_dark("#ffff00", m_useDarkStyle)));
  ...
  if (!m_mode.startsWith("WSPR"))
  {
    if (m_config.prompt_to_log())
    {
      qso_count_label->setStyleSheet(QString("QLabel{background: %1}").arg(Radio::convert_dark("#99ff99", m_useDarkStyle)));
    }
    else if (m_config.autolog())
    {
      qso_count_label->setStyleSheet(QString("QLabel{background: %1}").arg(Radio::convert_dark("#9999ff", m_useDarkStyle)));
    }
    else
    {
      qso_count_label->setStyleSheet(QString("QLabel{background: %1}").arg(Radio::convert_dark("#ffffff", m_useDarkStyle)));
    }
  }
  ...
  if (m_autoseq && !m_autoTx)
    ui->AutoTxButton->setStyleSheet(QString("QPushButton{color: %1;background: %2;border-style: solid;border-width: 1px;border-color: %3;min-width: 5em;padding: 3px}").arg(Radio::convert_dark("#000000", m_useDarkStyle), Radio::convert_dark("#ffbbbb", m_useDarkStyle), Radio::convert_dark("#808080", m_useDarkStyle)));
  else
    ui->AutoTxButton->setStyleSheet(QString("QPushButton:checked{background: %1}").arg(Radio::convert_dark("#00ff00", m_useDarkStyle)));
...
bool MainWindow::subProcessFailed(QProcess *process, int exit_code, QProcess::ExitStatus status)
{
  if (m_valid && (exit_code || QProcess::NormalExit != status))
  {
    ...
    JTDXMessageBox::critical_message(this, "", tr("Subprocess Error"), tr("Subprocess failed with exit code %1").arg(exit_code), tr("Running: %1\n%2").arg(process->program() + ' ' + arguments.join(' ')).arg(QString{process->readAllStandardError()}));
...
void MainWindow::subProcessError(QProcess *process, QProcess::ProcessError)
{
  if (m_valid)
  {
    ...
    JTDXMessageBox::critical_message(this, "", tr("Subprocess error"), tr("Running: %1\n%2").arg(process->program() + ' ' + arguments.join(' ')).arg(process->errorString()));
    QTimer::singleShot(0, this, SLOT(close()));
...
void MainWindow::on_pbSpotDXCall_clicked()
{
  if (m_config.spot_to_dxsummit() && !m_spotDXsummit && !m_hisCall.isEmpty() && !m_config.my_callsign().isEmpty())
  {
    ...
    QString spotInfoText = ui->spotMsgLabel->text();
    spotInfoText.remove(0, 6);
...
void MainWindow::msgBox(QString t)
{
  msgBox0.setText(t);
  msgBox0.translate_buttons();
  msgBox0.exec();
}
...
void MainWindow::diskDat() // diskDat()
{
  if (dec_data.params.kin > 0)
  {
    ...
    int kstep = m_FFTSize;
    m_diskData = true;
    for (int n = 1; n <= m_hsymStop; n++)
    { // Do the waterfall spectra
      k = (n + 1) * kstep;
...
void MainWindow::on_actionDelete_all_wav_files_in_SaveDir_triggered()
{
  ...
  {
    Q_FOREACH (auto const &file, m_config.save_directory().entryList({"*.wav", "*.c2"}, QDir::Files | QDir::Writable))
    {
...
void MainWindow::on_actionEstonian_triggered()
{
  ui->actionEstonian->setChecked(true);
  set_language("et_EE");
}
...
void MainWindow::on_actionCatalan_triggered()
{
  ui->actionCatalan->setChecked(true);
  set_language("ca_ES");
}
void MainWindow::on_actionCroatian_triggered()
{
  ui->actionCroatian->setChecked(true);
  set_language("hr_HR");
}
void MainWindow::on_actionDanish_triggered()
{
  ui->actionDanish->setChecked(true);
  set_language("da_DK");
}
void MainWindow::on_actionDutch_triggered()
{
  ui->actionDutch->setChecked(true);
  set_language("nl_NL");
}
...
{
  ui->actionSwedish->setChecked(true);
  set_language("sv_SE");
}
...
void MainWindow::on_actionLatvian_triggered()
{
  ui->actionLatvian->setChecked(true);
  set_language("lv_LV");
}
...
{
  ui->actionPortuguese->setChecked(true);
  set_language("pt_PT");
}
...
void MainWindow::on_actionChinese_simplified_triggered()
{
  ui->actionChinese_simplified->setChecked(true);
  set_language("zh_CN");
}
void MainWindow::on_actionChinese_traditional_triggered()
{
  ui->actionChinese_traditional->setChecked(true);
  set_language("zh_HK");
}
...
void MainWindow::on_actionCallMid_toggled(bool checked)
{
  ...
  if (checked)
  {
    ...
    if (!m_callPrioCQ)
      ui->AutoSeqButton->setText(tr("AutoSeq2"));
    else
      ui->AutoSeqButton->setText(tr("AutoSeq6"));
  }
}
...
void MainWindow::on_actionCallEnd_toggled(bool checked)
{
  ...
  if (checked)
  {
    ui->actionCallPriorityAndSearchCQ->setEnabled(true);
    if (!m_callPrioCQ)
      ui->AutoSeqButton->setText(tr("AutoSeq3"));
    else
      ui->AutoSeqButton->setText(tr("AutoSeq7"));
  }
}
...
{
  if (checked && m_rprtPriority)
  {
    ui->actionReport_message_priority->setChecked(false);
    m_rprtPriority = false;
  }
...
void MainWindow::on_actionEnable_hound_mode_toggled(bool checked)
{
  ...
  else
  {
    ...
    {
      m_skipTx1 = true;
      ui->skipTx1->setChecked(true);
      ui->skipGrid->setChecked(true);
      if (ui->txrb1->isChecked())
        on_txb2_clicked();
      if (ui->genMsg->text() == ui->tx1->text())
  ...
  if (!ui->spotLineEdit->text().isEmpty() && ui->spotLineEdit->text().contains("#H"))
    on_spotLineEdit_textChanged(ui->spotLineEdit->text());
}
...
void MainWindow::on_actionTxAfterQSO_toggled(bool checked) { m_tx_after_qso = checked; }
...
void MainWindow::on_actionShow_messages_decoded_from_harmonics_toggled(bool checked)
{
  if (checked)
  {
    dec_data.params.showharmonics = 1;
    m_showHarmonics = true;
  }
  else
  {
    dec_data.params.showharmonics = 0;
    m_showHarmonics = false;
  }
}
...
void MainWindow::on_actionFT8SensMin_toggled(bool checked)
{
  if (checked)
    m_ft8Sensitivity = 0;
}
void MainWindow::on_actionlowFT8thresholds_toggled(bool checked)
{
  if (checked)
    m_ft8Sensitivity = 1;
}
void MainWindow::on_actionFT8subpass_toggled(bool checked)
{
  if (checked)
    m_ft8Sensitivity = 2;
}
...
{
  if (!m_dataAvailable or m_TRperiod == 0.0)
  {
    m_manualDecode = false;
    return;
  }
  ...
  {
    if (m_curMsgTx.startsWith(m_hisCall + " "))
      dec_data.params.nstophint = 0; // DxCall matched to last transmitted message
    m_lasthint = false;
  }
  ...
  m_used_freq = 0;
  if (m_diskData && !m_mode.startsWith("FT"))
  ...
  if (dec_data.params.newdat == 1)
  {
    ...
    if (!m_mode.startsWith("FT"))
    {
      ...
      imin = imin % 60;
      if (m_TRperiod >= 60.0)
        imin = imin - (imin % (int(m_TRperiod) / 60));
  ...
  if (m_freqNominal < 30000000)
    dec_data.params.napwid = 5; // FT8AP decoding bandwidth for 'mycall hiscall ???' and RRR,RR73,73 messages
  else if (m_freqNominal < 100000000)
    dec_data.params.napwid = 15;
  else
  ...
  if (m_ft8Sensitivity == 0)
    dec_data.params.lft8lowth = false;
  else
    dec_data.params.lft8lowth = true;
  if (m_ft8Sensitivity == 2)
    dec_data.params.lft8subpass = true;
  else
    dec_data.params.lft8subpass = false;
  ...
  dec_data.params.lmodechanged = m_modeChanged ? 1 : 0;
  m_modeChanged = false;
  dec_data.params.lbandchanged = m_bandChanged ? 1 : 0;
  m_bandChanged = false;
  ...
  {
    hisCall.clear();
    strncpy(dec_data.params.hisbcall, (hisCall + "            ").toLatin1(), 12);
  }
  else
    strncpy(dec_data.params.hisbcall, (Radio::base_callsign(hisCall) + "            ").toLatin1(), 12);
  strncpy(dec_data.params.hisgrid, (hisGrid + "      ").toLatin1(), 6);

  if (!hisCall.isEmpty() && !m_enableTx && hisCall != m_lastloggedcall)
    dec_data.params.lenabledxcsearch = true;
  else
  ...
  if (m_config.write_decoded_debug())
  {
    ...
    {
      if (m_swl)
        cycles = "SWL On, cycles: " + QString::number(m_nFT8SWLCycles);
      else
        cycles = "SWL Off, cycles: " + QString::number(m_nFT8Cycles);
    }
    else
      swl = (m_swl ? "SWL On " : "SWL Off ");
...
void MainWindow::process_Auto()
{
  ...
  {
    if (m_houndMode)
      count = -1; // marker for changing status to FIN when status is RRR73
    ...
    if (m_config.write_decoded_debug())
    {
      ...
      if (m_status == QsoHistory::FIN)
        StrDirection = " auto sequence is finished;";
      else if (m_status == QsoHistory::NONE)
        StrDirection = " auto sequence is not started;";
    ...
    if (m_houndMode)
    { // WSJT-X Fox will drop QSO if R+Report from Hound is not decoded after three attempts
  ...
  if (hisCall.isEmpty() && counters && !m_houndMode && m_callMode != 0)
  {
    ...
    if (m_callPrioCQ && !m_lockTxFreq && counters2 && m_counter == 0 && m_txFirst != (nmod != 0))
    {
      time = 1;
    } // highiest priority, evaluating response to CQ first, then searching CQ decoded messages
    else
    {
      if (m_counter > 0)
        m_counter -= 1;
      time = 0;
    } // highiest priority, evaluating response to CQ only
    if ((!m_config.newDXCC() && !m_config.newGrid() && !m_config.newPx() && !m_config.newCall()) || m_answerWorkedB4)
      time |= 128;
    if ((!m_config.newDXCC() && !m_config.newGrid() && !m_config.newPx() && !m_config.newCall()) || m_callWorkedB4)
      time |= 64;
    else if (m_callHigherNewCall)
      time |= 256;
    if (m_rprtPriority)
      time |= 16;
    if (m_maxDistance)
    ...
    if (m_config.write_decoded_debug())
    {
      ...
      if (!hisCall.isEmpty())
      {
        if (prio > 27)
          ...
          StrPriority = " New ITUZ ";
        else if (prio > 19)
          StrPriority = " New DXCC ";
        else if (prio == 19 || prio == 4)
          StrPriority = " Wanted Call ";
        else if (prio == 18 || prio == 3)
          StrPriority = " Wanted Prefix ";
        else if (prio == 17 || prio == 2)
          StrPriority = " Wanted Country ";
    ...
    if (!hisCall.isEmpty())
    {
      if (m_callToClipboard)
        clipboard->setText(hisCall);
  ...
  if (m_status > QsoHistory::NONE)
  {
    ...
    if (time < 86400 && m_status < QsoHistory::R73)
    {
      ...
      if (m_jtdxtime->currentDateTimeUtc2() < m_dateTimeQSOOn)
        m_dateTimeQSOOn = m_dateTimeQSOOn.addSecs(-86400);
    }
    if (!rpt.isEmpty() && rpt != m_rpt)
    {
    ...
    {
      if (m_enableTx && m_autoseq && m_callMode > 0)
        txwatchdog(false);
      on_txb2_clicked();
      if (ui->tabWidget->currentIndex() == 1)
        ui->genMsg->setText(ui->tx2->text());
      if (m_autofilter && m_enableTx && !m_filter)
        autoFilter(true);
      break;
    }
  ...
  {
    if (m_enableTx && m_hisCall.isEmpty())
      ui->RxFreqSpinBox->setValue(ui->TxFreqSpinBox->value());
    if (!counters)
    {
      if (m_singleshot)
      {
        autoStopTx("m_singleshot, counter triggered ");
      }
      else if (m_houndMode)
      {
        autoStopTx("m_houndMode, counter triggered ");
      }
    }
  }
}
...
void MainWindow::readFromStdout() // readFromStdout
{
  while (proc_jtdxjt9.canReadLine())
  {
    ...
    if (t.startsWith("<DecodeFinished>"))
    {
      ...
      if (!m_diskData)
        killFileTimer.start(mswait); // Kill in 3/4 of period
      ...
      if (m_modeTx == "FT8")
        ...
        m_nguardfreq = 67;
      int indxncand = t.indexOf("<ncand>") + 7;
      m_ncand = t.mid(indxncand, 5).toInt();
      int indxdt = t.indexOf("<avexdt>") + 8;
      QString avexdt = t.mid(indxdt, 6).trimmed();
      ...
      if (m_autoseq && !m_processAuto_done && !m_manualDecode)
      {
      ...
      m_notified = false;
      if (m_config.write_decoded_debug())
        writeToALLTXT("Decoding finished");
      QString slag = "";
      ...
      if (!m_manualDecode && !m_diskData)
      {
        ...
        float lag = lagms / 1000.0;
        slag.setNum(lag, 'f', 2);
        if (lag > 0.0)
          slag = "+" + slag;
      }
      if (m_diskData && m_mode.startsWith("FT"))
      { // estimated lag for FT4||FT8 wav file
      ...
      if (m_mode.startsWith("FT"))
      {
        ...
        if (m_mode == "FT8")
        {
          if (!m_lostaudio)
          {
            if (navexdt < 76)
              ui->label_6->setStyleSheet(QString("QLabel{background: %1}").arg(Radio::convert_dark("#fdedc5", m_useDarkStyle)));
            else if (navexdt > 75 && navexdt < 151)
              ui->label_6->setStyleSheet(QString("QLabel{background: %1}").arg(Radio::convert_dark("#ffff00", m_useDarkStyle)));
            else if (navexdt > 150)
              ui->label_6->setStyleSheet(QString("QLabel{background: %1}").arg(Radio::convert_dark("#ff8000", m_useDarkStyle)));
            if (navexdt > 75)
              ui->label_6->setText(tr("check time"));
            else
              ui->label_6->setText(tr("Band Activity"));
          }
          else
          ...
          {
            if (navexdt > 19)
              m_jtdxtime->SetOffset(m_jtdxtime->GetOffset() - avexdt.toFloat());
            ui->syncButton->setChecked(false);
          }
          else if (!ui->syncButton->isEnabled())
        ...
        ui->decodedTextLabel->setText("UTC     dB   DT " + tr("Freq  ") + " " + tr("Lag=") + slag);
      dec_data.params.nagain = 0;
      dec_data.params.nagainfil = 0;
      dec_data.params.ndiskdat = 0;
      m_manualDecode = false;
    ...
    else
    {
      ...
      {
        QString band;
        if (m_jtdxtime->currentMSecsSinceEpoch2() / 1000 - m_secBandChanged > 50 || (m_jtdxtime->currentMSecsSinceEpoch2() / 1000 - m_secBandChanged > 14 && m_mode == "FT8") || (m_jtdxtime->currentMSecsSinceEpoch2() / 1000 - m_secBandChanged > 6 && m_mode == "FT4"))
        {
      ...
      if (!m_hisCall.isEmpty() && !deCall.isEmpty() && Radio::base_callsign(m_hisCall) == Radio::base_callsign(deCall))
      ...
      if (!deCall.isEmpty() && !m_reply_me && Radio::base_callsign(deCall) == Radio::base_callsign(m_hisCall))
      {
        ...
        else if (!m_reply_CQ73)
          m_reply_other = true;
      }
      // Left (Band activity) window
      bool bypassRxfFilters = false;
      int notified = 2;
      notified = ui->decodedTextBrowser->displayDecodedText(&decodedtext, m_baseCall, Radio::base_callsign(m_hisCall), m_hisGrid.left(4), m_notified, m_logBook, m_qsoHistory, m_qsoHistory, m_freqNominal, m_mode, bypassRxfFilters, m_bypassAllFilters, m_wideGraph->rxFreq(), m_wantedCallList, m_wantedPrefixList, m_wantedGridList, m_wantedCountryList, m_windowPopup, this);
      if (m_position != 0)
        ui->decodedTextBrowser->horizontalScrollBar()->setValue(m_position);
      if (notified & 1)
        m_notified = true;
      ...
      if (message_words.length() == 4 && !m_enableTx)
      {
        ...
        if (!dxbasecall.isEmpty() && message_words.at(2).contains(dxbasecall) && message_words.at(3).contains("73"))
          bdxcall73 = true;
      }
      ...
      if (qAbs(decodedtext.frequencyOffset() - m_wideGraph->rxFreq()) <= 10 || (m_showMyCallMsgRxWindow && mycallinmsg) || bcontent || bdxcall73 || (m_showWantedCallRxWindow && notified & 8))
      {
        ...
        ui->decodedTextBrowser2->displayDecodedText(&decodedtext, m_baseCall, Radio::base_callsign(m_hisCall), m_hisGrid.left(4), m_notified, m_logBook, m_qsoHistory2, m_qsoHistory, m_freqNominal, m_mode, bypassRxfFilters, m_bypassAllFilters, m_wideGraph->rxFreq());
      ...
      if (mycallinmsg && !m_manualDecode)
      {
        ...
        {
          if (!m_processAuto_done && m_autoseq && ((!decodedtextmsg.contains(" 73") && !decodedtextmsg.contains("RR73")) || (decodedtextmsg.contains(" 73") && m_status == QsoHistory::RRREPORT && m_rrr) || (decodedtextmsg.contains("RR73") && m_status == QsoHistory::RREPORT) || m_callMode == 0 || m_singleshot || m_houndMode))
          {
            m_processAuto_done = true;
            process_Auto();
          }
          else if ((decodedtextmsg.contains(" 73") || decodedtextmsg.contains("RR73")) && m_callMode <= 1)
            m_callFirst73 = true;
        }
      ...
      if ((!m_config.prevent_spotting_false() || (m_config.prevent_spotting_false() && !decodedtext.isWrong())) && (!m_config.filterUDP() || (m_config.filterUDP() && notified & 2)))
      {
...
void MainWindow::killFile()
{
  ...
  if (m_logInitNeeded)
  {
    ...
    if (m_config.write_decoded_debug())
      writeToALLTXT("Log initialization is started: wsjtx_log.adi file was changed");
...
void MainWindow::set_language(QString const &lang)
{
  if (m_lang != lang)
  {
    ...
    if (olek)
    {
      ...
      if (!tolge.isEmpty())
        msgbox.setWindowTitle(tolge);
      else
        msgbox.setWindowTitle("Confirm change Language");
      tolge = translator.translate("MainWindow", "Are You sure to change UI Language to English, JTDX will restart?");
      if (!tolge.isEmpty())
        msgbox.setText(tolge);
      else
        msgbox.setText("Are You sure to change UI Language to English, JTDX will restart?");
      tolge = translator.translate("JTDXMessageBox", "&Yes");
      if (!tolge.isEmpty())
        msgbox.button(JTDXMessageBox::Yes)->setText(tolge);
      else
        msgbox.button(JTDXMessageBox::Yes)->setText("&Yes");
      tolge = translator.translate("JTDXMessageBox", "&No");
      if (!tolge.isEmpty())
        msgbox.button(JTDXMessageBox::No)->setText(tolge);
      else
        msgbox.button(JTDXMessageBox::No)->setText("&No");
    }
  ...
  if (m_lang == "et_EE")
    ui->actionEstonian->setChecked(true);
  else if (m_lang == "ru_RU")
    ui->actionRussian->setChecked(true);
  else if (m_lang == "ca_ES")
    ui->actionCatalan->setChecked(true);
  else if (m_lang == "hr_HR")
    ui->actionCroatian->setChecked(true);
  else if (m_lang == "da_DK")
    ui->actionDanish->setChecked(true);
  else if (m_lang == "nl_NL")
    ui->actionDutch->setChecked(true);
  else if (m_lang == "es_ES")
    ui->actionSpanish->setChecked(true);
  else if (m_lang == "fr_FR")
    ui->actionFrench->setChecked(true);
  else if (m_lang == "hu_HU")
    ui->actionHungarian->setChecked(true);
  else if (m_lang == "it_IT")
    ui->actionItalian->setChecked(true);
  else if (m_lang == "lv_LV")
    ui->actionLatvian->setChecked(true);
  else if (m_lang == "pl_PL")
    ui->actionPolish->setChecked(true);
  else if (m_lang == "pt_PT")
    ui->actionPortuguese->setChecked(true);
  else if (m_lang == "pt_BR")
    ui->actionPortuguese_BR->setChecked(true);
  else if (m_lang == "sv_SE")
    ui->actionSwedish->setChecked(true);
  else if (m_lang == "zh_CN")
    ui->actionChinese_simplified->setChecked(true);
  else if (m_lang == "zh_HK")
    ui->actionChinese_traditional->setChecked(true);
  else if (m_lang == "ja_JP")
    ui->actionJapanese->setChecked(true);
  else
    ui->actionEnglish->setChecked(true);
}
...
void MainWindow::guiUpdate()
{
  ...
  txDuration = 0.0;
  if (m_modeTx == "FT8")
    txDuration = 13.64; // 1.0 + 79*1920/12000.0;
  else if (m_modeTx == "FT4")
    txDuration = 6.04; // 1.0 + 105*576/12000.0;
  else if (m_modeTx == "JT65")
    txDuration = 47.81142857142857; // 1.0 + 126*4096/11025.0;
  else if (m_modeTx == "JT9")
    txDuration = 49.96; // 1.0 + 85.0*m_nsps/12000.0;
  else if (m_modeTx == "T10")
    txDuration = 49.96; // 1.0 + 85.0*m_nsps/12000.0;
  else if (m_mode == "WSPR-2")
    txDuration = 112.592; // 2.0 + 162*8192/12000.0;
  ...
  if (icw[0] > 0)
    tx2 += icw[0] * 2560.0 / 48000.0; // Full length including CW ID
  if (tx2 > m_TRperiod)
    tx2 = m_TRperiod;
  ...
  if (m_mode.left(4) == "WSPR")
  {
    ...
    {                   // Decide whether to Tx or Rx
      ...
      if (m_pctx == 0)
        m_WSPR_tx_next = false;                // Don't transmit if m_pctx=0
      bool btx = m_enableTx && m_WSPR_tx_next; // To Tx, we need m_enableTx and
                                               // scheduled transmit
      if (m_enableTx and m_txNext)
        btx = true; // TxNext button overrides
      if (m_enableTx and m_pctx == 100)
        btx = true; // Always transmit
  ...
  if (m_tune)
    m_bTxTime = true; //"Tune" takes precedence

  if (m_transmitting or m_enableTx or m_tune)
  {
    // Check for "txboth" (testing purposes only)
    QFile f(m_appDir + "/txboth");
    if (f.exists() and fmod(tsec, m_TRperiod) < 49.96)
      m_bTxTime = true; //<(1.0 + 85.0*m_nsps/12000.0)
    ...
    if (g_iptt == 0 and ((m_bTxTime and fTR < 99.0) or m_tune))
    { // ### Allow late starts
      icw[0] = m_ncw;
      g_iptt = 1;
      setRig();
  ...
  if ((g_iptt == 1 && iptt0 == 0) || m_restart)
  {
    ...
    else
    {
      ...
      {
        txMsg = ui->genMsg->text();
      }
      else if (m_ntx == 8)
      {
        txMsg = ui->freeTextMsg->currentText();
        txMsg.remove(" ^");
      }
      ...
      if (!m_tune && (msgLength == 0 || m_config.my_callsign().isEmpty() || (m_houndMode && (m_QSOProgress == CALLING || m_ntx == 6))))
      {
        haltedEmpty = true;
        if (m_houndMode)
          m_currentMessage = txMsg;
        else
          m_currentMessage.clear();
        if (msgLength == 0)
          haltTx("Transmission of the empty message is not allowed ");
        else if (m_config.my_callsign().isEmpty())
          haltTx("Transmission halted: user callsign is not configured ");
        else
          haltTx("Transmission of CQ message is not allowed in the Hound mode ");
      }
      else
      {
        ba = txMsg.toLocal8Bit();
      }
    }

    ba2msg(ba, message);
    int ichk = 0;
    if (m_lastMessageSent != m_currentMessage || m_lastMessageType != m_currentMessageType)
    {
    ...
    else
    {
      ...
      {
        gen65_(message, &ichk, msgsent, const_cast<int *>(itone), &m_currentMessageType, len1, len1);
      }
      else if (m_modeTx == "JT9")
      {
        gen9_(message, &ichk, msgsent, const_cast<int *>(itone), &m_currentMessageType, len1, len1);
      }
      else if (m_modeTx == "T10")
      {
        gen10_(message, &ichk, msgsent, const_cast<int *>(itone), &m_currentMessageType, len1, len1);
      }
      else if (m_mode.left(4) == "WSPR")
      {
        genwspr_(message, msgsent, const_cast<int *>(itone), len1, len1);
      }

      if (m_colorTxMsgButtons && m_mode.left(4) != "WSPR")
      {
        if (m_restart)
          resetTxMsgBtnColor();
        setTxMsgBtnColor();
    ...
    if (m_mode.startsWith("FT"))
    {
      m_curMsgTx.remove(QChar{'<'});
      m_curMsgTx.remove(QChar{'>'});
    }
    m_currentMessage = m_curMsgTx;
    if (m_tune)
    {
      m_currentMessage = tr("TUNE");
      m_currentMessageType = -1;
      m_nlasttx = 0;
    }
    ...
    if (m_restart && !haltedEmpty)
    {
      ...
      if (f.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Append))
      {
        ...
        if (m_config.write_decoded_debug())
        {
          QString autoseqa = ui->AutoSeqButton->text();
          int indx = autoseqa.length() - 1;
          QString autoseq = "AutoSeq" + autoseqa[indx];
          out << m_jtdxtime->currentDateTimeUtc2().toString("yyyyMMdd_hhmmss.zzz") << "(" << m_jtdxtime->GetOffset() << ")"
      ...
      if (m_config.TX_messages() || m_autoseq)
      {
        if (0 == ui->tabWidget->currentIndex())
        {
          if (ui->txrb5->isChecked() && m_autoseq && m_Tx5setAutoSeqOff)
          {
            m_wasAutoSeq = true;
            on_AutoSeqButton_clicked(false);
          }
        }
        else if (1 == ui->tabWidget->currentIndex())
        {
          if (ui->rbFreeText->isChecked() && m_autoseq && m_FTsetAutoSeqOff)
          {
            m_wasAutoSeq = true;
            on_AutoSeqButton_clicked(false);
          }
    ...
    m_sentFirst73 = is_73 && m_lastloggedcall != m_hisCall && (!lastmsg_is73 || (lastmsg_is73 && !m_lastMessageSent.contains(m_hisCall)));

    if (m_sentFirst73)
    {
      if (!m_mode.startsWith("FT") && m_config.id_after_73())
        icw[0] = m_ncw;
    ...
    if (m_currentMessageType < 6 && msg_parts.length() >= 3 && (msg_parts[1] == m_config.my_callsign() || msg_parts[1] == m_baseCall))
    {
      int i1;
      bool ok;
  ...
  if (g_iptt == 1 && iptt0 == 0)
  {
    if (m_config.watchdog() && !m_mode.startsWith("WSPR") && m_curMsgTx != m_msgSent0)
    {
    ...
    if (m_autoseq && (m_QSOProgress == CALLING || m_ntx == 6) && !m_hisCall.isEmpty())
      clearDXfields(" field cleared, CQ message triggered by AutoSeq");
    if (m_houndMode && (m_QSOProgress == REPLYING || m_ntx == 1))
      m_lastCallingFreq = ui->TxFreqSpinBox->value();
    if (m_curMsgTx != m_msgSent0)
      m_msgSent0 = m_curMsgTx;
    ...
    if ((m_config.TX_messages() || m_autoseq) && !m_tune)
    {
      ...
      if (!haltedEmpty)
        ui->decodedTextBrowser2->displayTransmittedText(m_curMsgTx, m_baseCall, Radio::base_callsign(m_hisCall), (m_skipTx1 && !m_houndMode ? "S" : ""), m_modeTx,
                                                        ui->TxFreqSpinBox->value(), m_config.color_TxMsg(), m_qsoHistory);
    }
    switch (m_ntx)
    {
    ...
    case 6:
      ...
      break;
    default:
      break; // determined elsewhere
    }

    m_transmitting = true;
    if (!m_config.tx_QSY_allowed())
      ui->TxFreqSpinBox->setDisabled(true);
    ...
    if (m_filter)
    {
      ...
      {
        if (m_QSOProgress == SIGNOFF || (!m_rrr && m_QSOProgress == ROGERS))
          autoFilter(false);
  ...
  if ((!m_btxok && btxok0 && g_iptt == 1) || m_haltTrans)
  {
    stopTx();
    if (m_haltTrans)
      m_haltTrans = false;
  ...
  if (nsec != m_sec0)
  {
    if (m_config.watchdog() && !m_transmitting && !m_mode.startsWith("WSPR") && m_idleMinutes >= m_config.watchdog())
    {
      txwatchdog(true); // switch off Enable Tx button
    }
    if (m_tune && m_config.tunetimer() && !m_tuneup)
    { // shall not count at WSPR band hopping
      QString remtime;
    ...
    if (m_transmitting)
    {
      ...
      if (m_tune)
        tx_status_label->setText(tr("Tx: TUNE"));
      else
        tx_status_label->setText(tr("Tx: ") + m_curMsgTx.trimmed());
    ...
    if (m_transmitting && !m_tune && (m_nseq == 10 || m_nseq == 11))
    {
      m_lapmyc = 1;
      m_mslastTX = m_jtdxtime->currentMSecsSinceEpoch2();
    } // setting twice: make sure it is not skipped
    ...
    if ((m_mode == "FT8" && isecond % 15 == 0) ||
        ...
        (!m_mode.startsWith("FT") && second == "00"))
      setClockStyle(false);
...
void MainWindow::set_scheduler(QString const &setto, bool mixed)
{
  ...
  if (setto.contains(","))
  {
    frq = setto.left(setto.indexOf(" ")).replace("*", "").replace(",", "").toInt();
  }
  else
  {
    frq = setto.left(setto.indexOf(" ")).replace("*", "").replace(".", "").toInt();
  ...
  band_changed(frq);
  if (m_config.write_decoded_debug())
    writeToALLTXT("Band changed from scheduler, frequency: " + QString::number(frq));
...
void MainWindow::haltTxTuneTimer()
{
  if (m_config.write_decoded_debug())
  {
    writeToALLTXT("Halt Tx triggered: tuning stopped");
    m_haltTxWritten = true;
  }
  on_stopTxButton_clicked();
}
...
// workaround to handle file overwriting scenario under Linux
    ...
    int counter = 0;
    while (!checkFile.exists() && counter < 15)
    {
...
void MainWindow::startTx2()
{
  ...
  { // TODO - not thread safe
    ...
    if (t.left(1) == "#")
      fSpread = t.mid(1, 5).toDouble();
    if (tci_active)
      Q_EMIT m_config.transceiver_spread(fSpread);
    else
      m_modulator->setSpread(fSpread); // TODO - not thread safe
    t = ui->tx6->text();
    if (t.left(1) == "#")
      snr = t.mid(1, 5).toDouble();
    if (snr > 0.0 or snr < -50.0)
      snr = 99.0;
    transmit(snr);
    //      printf(" started %s\n",m_jtdxtime->currentDateTimeUtc2().toString("hh:mm:ss.zzz").toStdString().c_str());
    if (m_config.write_decoded_debug())
      writeToALLTXT("Modulator started");
    //      QThread::currentThread()->setPriority(QThread::HighPriority);
    ui->signal_meter_widget->setValue(0);
  ...
  if (m_config.write_decoded_debug())
    writeToALLTXT("MainWindow::startTx2() failed to start modulator: m_modulator is active");
}

void MainWindow::stopTx()
{
  if (m_tci)
    Q_EMIT m_config.transceiver_modulator_stop();
  else
...
{
  m_txFirst = checked;
  if (m_transmitting && m_config.write_decoded_debug())
    writeToALLTXT("Tx halted: period changed via TX period button");
  if (m_txGenerated != checked && m_enableTx && m_autoseq)
    clearDX(" cleared, Tx Minute button clicked");
  setMinButton();
}
...
void MainWindow::on_rrrCheckBox_stateChanged(int nrrr) // RRR or RR73
{
  ...
  ui->rrr1CheckBox->setChecked(m_rrr);
  if (ui->genMsg->text().contains(" RRR") || ui->genMsg->text().contains(" RR73"))
    on_pbSendRRR_clicked();
}
...
void MainWindow::on_rrr1CheckBox_stateChanged(int nrrr) // RRR or RR73
{
  m_rrr = (nrrr == 2);
  ui->rrrCheckBox->setChecked(m_rrr);
  if (!ui->tx4->text().isEmpty())
    genStdMsgs(m_rpt); // can change message for non tx4 transmission?
}

void MainWindow::set_ntx(int n)
{
  m_ntx = n;
  m_nlasttx = n;
} // set_ntx()
...
void MainWindow::on_txb6_clicked() // txb6
{
  if (!m_hisCall.isEmpty())
    clearDXfields(" field cleared, SLOT on_txb6_clicked()");
  ...
  if (m_spotDXsummit)
  {
    ...
    if (m_config.spot_to_dxsummit())
    {
      ui->pbSpotDXCall->setStyleSheet(QString("QPushButton {color: 51;background: %2;border-style: outset;border-width: 1px;border-color: %3;padding: 3px}").arg(Radio::convert_dark("#000000", m_useDarkStyle), Radio::convert_dark("#c4c4ff", m_useDarkStyle), Radio::convert_dark("#808080", m_useDarkStyle)));
    }
    else
    {
      ui->pbSpotDXCall->setStyleSheet(QString("QPushButton {color: %1;background: %2;border-style: outset;border-width: 1px;border-color: %3;padding: 3px}").arg(Radio::convert_dark("#000000", m_useDarkStyle), Radio::convert_dark("#aabec8", m_useDarkStyle), Radio::convert_dark("#808080", m_useDarkStyle)));
...
void MainWindow::doubleClickOnCall(bool alt, bool ctrl)
{
  ...
  QString messages;
  if (!m_decodedText2)
    messages = ui->decodedTextBrowser2->toPlainText();
  if (m_decodedText2)
    messages = ui->decodedTextBrowser->toPlainText();
...
void MainWindow::processMessage(QString const &messages, int position, bool alt, bool ctrl)
{
  ...
  if (!addWanted)
  {
    ...
    if (m_txFirst != (nmod != 0) && !tx_message)
    {
      ...
      if (m_txFirst && !ui->TxMinuteButton->isChecked())
        ui->TxMinuteButton->click();
      else if (!m_txFirst && ui->TxMinuteButton->isChecked())
        ui->TxMinuteButton->click();
      //      setMinButton();
      period_changed = true;
      if (m_transmitting && m_config.write_decoded_debug())
        writeToALLTXT("Tx halted: period changed via message click");
    }
  }
  ...
  if (t4.size() < 6)
    return; // Skip the rest if no decoded text
  ...
  if (addWanted)
  {
    if (!hiscall.isEmpty() && hiscall != m_hisCall && hiscall != m_config.my_callsign())
    {
      ...
      if (m_mode.startsWith("FT"))
        ...
        t5 = Radio::base_callsign(hiscall);
      QString t6{""};
      if (m_wantedCall.isEmpty())
        ui->wantedCall->setText(t5);
      else if (m_wantedCallList.length() < 20 && !m_wantedCall.startsWith(t5 + ",") && !m_wantedCall.endsWith("," + t5) && !m_wantedCall.contains("," + t5 + ","))
      {
        t6 = m_wantedCall;
        ui->wantedCall->setText(t6.append("," + t5));
      }
    }
    return;
  }
  ...
  if (!Radio::is_callsign(hiscall)    // not interested if not from QSO partner
      && !(t4.size() == 7             // unless it is of the form
           && (t4.at(5) == m_baseCall // "<our-call> 73"
               || t4.at(5).startsWith(m_baseCall + '/') || t4.at(5).endsWith('/' + m_baseCall)) &&
           t4.at(6) == "73"))
  {
  ...
  if (i9 < 0 and !decodedtext.isTX() and m_decodedText2)
  {
    ...
    {
      ui->decodedTextBrowser2->displayDecodedText(&decodedtext, m_baseCall, Radio::base_callsign(m_hisCall), m_hisGrid.left(4), m_notified, m_logBook, m_qsoHistory2, m_qsoHistory, m_freqNominal, m_mode, m_bypassRxfFilters, m_bypassAllFilters, m_wideGraph->rxFreq());
    }
    m_QSOText = decodedtext.string();
  }
  ...
  if (dtext.contains(" " + m_baseCall + " ") || dtext.contains("/" + m_baseCall + " ") || dtext.contains(" " + m_baseCall + "/") || (firstcall == "DE" && ((t4.size() > 7 && t4.at(7) != "73") || t4.size() <= 7)))
  {
  ...
  if (m_autofilter && m_autoseq && !m_filter && (m_QSOProgress == REPORT || m_QSOProgress == REPLYING))
    autoFilter(true);
  if (m_transmitting && !period_changed)
    m_restart = true;
  //  if(m_autoTx && !alt) enableTx_mode(true);
  // an attempt to provide stable UDP Reply operation:
  if (m_autoTx && !alt)
  {
    if (!ui->enableTxButton->isChecked())
      ui->enableTxButton->click();
  }
  if (alt && m_enableTx)
  ...
  if (m_config.write_decoded_debug())
  {
    ...
    QString AuTXstate = m_autoTx ? "On" : "OFF";
    writeToALLTXT("double click on call processed: " + hiscall + "; EnableTx " + EnTXstate + ", AutoTx " + AuTXstate);
  }
  if (!ui->spotLineEdit->text().isEmpty() && ui->spotLineEdit->text().contains("#"))
    on_spotLineEdit_textChanged(ui->spotLineEdit->text());
}
...
void MainWindow::genCQMsg()
{
  if (m_config.my_callsign().size() && m_config.my_grid().size())
  {
    ...
    if (!m_cqdir.isEmpty() && stdCall(m_config.my_callsign()))
    {
      QString t = "CQ " + m_cqdir + " " + m_config.my_callsign() + " " + m_config.my_grid().left(4);
      msgtype(t, ui->tx6);
    }
...
void MainWindow::genStdMsgs(QString rpt) // genStdMsgs()
{
  ...
  bool bothCallsP = (my_callsign.endsWith("/P") and hisCall.endsWith("/P"));
  bool bothCallsR = (my_callsign.endsWith("/R") and hisCall.endsWith("/R"));
  ...
  {
    QString t0a, t0b;
    if (m_bHisCallStd and m_bMyCallStd)
      t0 = hisCall + " " + my_callsign + " ";
    ...
    if (!mixedPR)
    {
      ...
      {
        t = t0a;
        if (m_bMyCallStd && !my_callsign.endsWith("/P"))
    ...
    else if ((m_bHisCallStd and !m_bMyCallStd))
      t = "<" + hisCall + "> " + my_callsign + " " + (m_rrr ? "RRR" : "RR73");
    msgtype(t, ui->tx4);
  ...
  if (m_config.my_callsign() != m_baseCall)
  {
    ...
    if (hisCall != hisBase && m_config.type_2_msg_gen() != Configuration::type_2_msg_5_only)
    {
...
void MainWindow::clearDX(QString reason)
{
  ...
  if (!reason.isEmpty() && m_config.write_decoded_debug())
    writeToALLTXT("DX Call " + dxcallclr + reason);
}
...
void MainWindow::countQSOs()
{
  ...
  if (m_mode == "FT8")
  {
    sprintf(c_txt, "FT8  %d", m_logBook.get_qso_count("FT8"));
  }
  else if (m_mode == "FT4")
  {
    sprintf(c_txt, "FT4  %d", m_logBook.get_qso_count("FT4"));
  }
  else if (m_mode == "JT9+JT65")
  {
    sprintf(c_txt, "JT65/9 %d/%d", m_logBook.get_qso_count("JT65"), m_logBook.get_qso_count("JT9"));
  }
  else if (m_mode == "JT9")
  {
    sprintf(c_txt, "JT9  %d", m_logBook.get_qso_count("JT9"));
  }
  else if (m_mode == "JT65")
  {
    sprintf(c_txt, "JT65  %d", m_logBook.get_qso_count("JT65"));
  }
  else if (m_mode == "T10")
  {
    sprintf(c_txt, "T10  %d", m_logBook.get_qso_count("T10"));
  }
  else
  {
    sprintf(c_txt, " ");
...
{
  QString hisCall = m_hisCall;
  if (hisCall.isEmpty() || hisCall.endsWith("/P") || hisCall.endsWith("/MM") || hisCall.endsWith("/A") || hisCall.endsWith("/M"))
    return;
...
void MainWindow::on_addButton_clicked()
{
  if (m_hisGrid.isEmpty())
  {
    JTDXMessageBox::warning_message(this, "", tr("Add to CALL3.TXT"), tr("Please enter a valid grid locator"));
    return;
  }
  ...
  {
    JTDXMessageBox::warning_message(this, "", tr("Add to CALL3.TXT"), tr("Cannot open \"%1\" for read/write: %2").arg(f1.fileName()).arg(f1.errorString()));
    return;
  }
  if (f1.size() == 0)
  {
  ...
  {
    JTDXMessageBox::warning_message(this, "", tr("Add to CALL3.TXT"), tr("Cannot open \"%1\" for writing: %2").arg(f2.fileName()).arg(f2.errorString()));
    return;
  }
...
void MainWindow::on_tx1_editingFinished()
{
  QString t = ui->tx1->text();
  msgtype(t, ui->tx1);
}
void MainWindow::on_tx2_editingFinished()
{
  QString t = ui->tx2->text();
  msgtype(t, ui->tx2);
}
void MainWindow::on_tx3_editingFinished()
{
  QString t = ui->tx3->text();
  msgtype(t, ui->tx3);
}
void MainWindow::on_tx4_editingFinished()
{
...
void MainWindow::on_tx5_currentTextChanged(QString const &text) // tx5 edited
{
  ...
  if (!m_Tx5setAutoSeqOff && !isAllowedAuto73)
    m_Tx5setAutoSeqOff = true;
  if (isAllowedAuto73)
    m_Tx5setAutoSeqOff = false;
  if (!text.contains(QRegularExpression{R"([@#&^])"}) && !text.isEmpty())
  {
    ...
    else
    {
      ...
      {
        ui->tx5->setStyleSheet(QString("QComboBox {background: %1}").arg(Radio::convert_dark("#ffff00", m_useDarkStyle)));
      }
      msgtype(text, ui->tx5->lineEdit());
    }
  }
  else
    ui->tx5->setStyleSheet(QString("QComboBox {background: %1}").arg(Radio::convert_dark("#ffffff", m_useDarkStyle)));
}
...
void MainWindow::on_tx5_currentIndexChanged(int index)
{
  ...
  if (m_transmitting && ui->txrb5->isChecked() && index != m_oldTx5Index)
    m_restart = true;
  m_oldTx5Index = index;
}

void MainWindow::on_tx6_editingFinished()
{
  QString t = ui->tx6->text();
  msgtype(t, ui->tx6);
}
...
void MainWindow::on_directionLineEdit_textChanged(const QString &dir) // CQ direction changed
{
  ...
  if (cqdirection.isEmpty())
  {
    ui->directionLineEdit->setStyleSheet(QString("QLineEdit {background: %1}").arg(Radio::convert_dark("#ffffff", m_useDarkStyle)));
  }
  else if (cqdirection.length() == 1 || cqdirection.length() > 2)
  {
    ui->directionLineEdit->setStyleSheet(QString("QLineEdit {background: %1}").arg(Radio::convert_dark("#fffa82", m_useDarkStyle)));
  }
  else if (cqdirection.length() == 2)
  {
    ui->directionLineEdit->setStyleSheet(QString("QLineEdit {background: %1}").arg(Radio::convert_dark("#82ff8c", m_useDarkStyle)));
  }
  m_cqdir = cqdirection;
  if (cqdirection.isEmpty())
  {
  ...
  auto curpos = ui->directionLineEdit->cursorPosition();
  ui->directionLineEdit->setText(m_cqdir);
  ui->direction1LineEdit->setText(m_cqdir);
  ui->directionLineEdit->setCursorPosition(curpos);
...
void MainWindow::on_direction1LineEdit_textChanged(const QString &dir) // CQ direction changed
{
  ...
  if (cqdirection.isEmpty())
  {
    ui->direction1LineEdit->setStyleSheet(QString("QLineEdit {background: %1}").arg(Radio::convert_dark("#ffffff", m_useDarkStyle)));
  }
  else if (cqdirection.length() == 1 || cqdirection.length() > 2)
  {
    ui->direction1LineEdit->setStyleSheet(QString("QLineEdit {background: %1}").arg(Radio::convert_dark("#fffa82", m_useDarkStyle)));
  }
  else if (cqdirection.length() == 2)
  {
    ui->direction1LineEdit->setStyleSheet(QString("QLineEdit {background: %1}").arg(Radio::convert_dark("#82ff8c", m_useDarkStyle)));
  }
  m_cqdir = cqdirection;
  if (cqdirection.isEmpty())
  {
  ...
  auto curpos = ui->direction1LineEdit->cursorPosition();
  ui->direction1LineEdit->setText(m_cqdir);
  ui->directionLineEdit->setText(m_cqdir);
  ui->direction1LineEdit->setCursorPosition(curpos);
  if (cqdirection.isEmpty() || cqdirection.length() == 2)
  {
    if (ui->tx1->text().isEmpty())
    {
      if (ui->txrb6->isChecked())
        ui->txb6->click();
    }
    else
    {
      if (cqdirection.length() == 2)
        ui->tx6->setText("CQ " + m_cqdir + " " + m_config.my_callsign() + " " + m_config.my_grid().left(4));
      else
        ui->tx6->setText("CQ " + m_config.my_callsign() + " " + m_config.my_grid().left(4));
    }
  }
}
...
void MainWindow::on_spotLineEdit_textChanged(const QString &text)
{
  ...
  QString spotText;
  if (ui->spotLineEdit->text().isEmpty() && ui->propLineEdit->text().isEmpty())
  {
    spotText = "info: " + m_modeTx;
  }
  else if (ui->spotLineEdit->text().isEmpty() && !ui->propLineEdit->text().isEmpty())
  {
    spotText = "info: " + m_modeTx + " " + ui->propLineEdit->text();
  }
  else if (!m_spotText.contains("#G", Qt::CaseInsensitive))
  {
    spotText = "info: " + m_modeTx + " " + ui->propLineEdit->text() + " " + spotTextTmp;
  }
  else
  {
    spotText = "info: " + m_modeTx + " " + spotTextTmp;
  }
...
void MainWindow::on_propLineEdit_textChanged(const QString &text)
{
  if (text != text.toUpper())
    ui->propLineEdit->setText(text.toUpper());
  on_spotLineEdit_textChanged(ui->spotLineEdit->text());
}
...
void MainWindow::on_dxCallEntry_textChanged(const QString &t) // dxCall changed
{
  ...
  if (n < 3)
  {
    if (t != t.toUpper().trimmed())
      ui->dxCallEntry->setText(t.toUpper().trimmed());
    if (m_hisCall.isEmpty())
    {
      m_hisCallCompound = false;
      return;
  ...
  if (t != m_hisCall && !m_hisCall.isEmpty())
  {
    ui->dxCallEntry->setText(m_hisCall);
    ui->dxCallEntry->setCursorPosition(pos);
  }
  else
  {
    QString grid;
    ...
    ui->dxCallEntry->setStyleSheet(QString("QLineEdit {color: %1; background: %2}").arg(Radio::convert_dark("#000000", m_useDarkStyle), Radio::convert_dark("#ffffff", m_useDarkStyle)));
    if (logClearDXTimer.isActive())
      logClearDXTimer.stop();
    // Refresh Tx macros
    ...
    {
      name = name.toUpper();
      QRegularExpression name_re("^[A-Z]{2,7}$");
      QRegularExpressionMatch match = name_re.match(name);
      bool hasMatch = match.hasMatch();
      if (hasMatch)
        list2 = list2.replaceInStrings("^", name);
    }
...
void MainWindow::on_dxGridEntry_textChanged(const QString &t) // dxGrid changed
{
  ...
  if (n != 4 and n != 6 and n != 8 and n != 10)
  {
    if (n < 4 || n == 5)
    {
      if (t != t.left(2).toUpper() + t.mid(2, 2) + t.mid(4, 1).toLower())
        ui->dxGridEntry->setText(t.left(2).toUpper() + t.mid(2, 2) + t.mid(4, 1).toLower());
      if (n < 4 && !m_hisGrid.isEmpty())
      {
        ui->labAz->clear();
        ui->labDist->clear();
        m_hisGrid.clear();
    ...
    {
      if (t != t.left(2).toUpper() + t.mid(2, 2) + t.mid(4, 2).toLower() + t.mid(6, 1))
        ui->dxGridEntry->setText(t.left(2).toUpper() + t.mid(2, 2) + t.mid(4, 2).toLower() + t.mid(6, 1));
    }
    else if (n == 9)
    {
      if (t != t.left(2).toUpper() + t.mid(2, 2) + t.mid(4, 2).toLower() + t.mid(6, 2) + t.mid(8, 1).toLower())
        ui->dxGridEntry->setText(t.left(2).toUpper() + t.mid(2, 2) + t.mid(4, 2).toLower() + t.mid(6, 2) + t.mid(8, 1).toLower());
    }
  ...
  if (t != m_hisGrid)
  {
    ...
    ui->dxGridEntry->setCursorPosition(pos);
  }
  else
  {
    statusUpdate();
    ...
    if (m_config.miles())
    {
      t = QString::asprintf("%d mi", nDmiles);
    }
    else
    {
      t = QString::asprintf("%d km", nDkm);
    }
...
void MainWindow::on_logQSOButton_clicked()
{
  ...
  {
    if (m_mode == "FT8")
      dateTimeQSOOff = currenttime.addSecs(14);
    else if (m_mode == "FT4")
      dateTimeQSOOff = currenttime.addSecs(7);
    else
      dateTimeQSOOff = currenttime.addSecs(50);
  }
  if (dateTimeQSOOff < m_dateTimeQSOOn)
    m_dateTimeQSOOn = dateTimeQSOOff;
  // 75 for FT4, 150 seconds delta for FT8, 600 seconds delta for other modes
  if (qAbs(dateTimeQSOOff.toMSecsSinceEpoch() - m_dateTimeQSOOn.toMSecsSinceEpoch()) > int(m_TRperiod) * 10000)
    m_dateTimeQSOOn = dateTimeQSOOff;
  bool autolog = false;
  if (m_logqso73)
    autolog = m_config.autolog();
  ...
  if (m_qsoHistory.log_data(m_hisCall, time, rrep, srep) > QsoHistory::SREPORT)
  {
    if (time < 86400)
    {
      ...
      if (dateTimeQSOOff < currenttime)
        currenttime = currenttime.addSecs(-86400);
    }
    if (rrep.isEmpty())
      rrep = m_rptRcvd;
    if (srep.isEmpty())
...
void MainWindow::acceptQSO2(QDateTime const &QSO_date_off, QString const &call, QString const &grid, Frequency dial_freq, QString const &mode, QString const &rpt_sent, QString const &rpt_received, QString const &tx_power, QString const &comments, QString const &name, QDateTime const &QSO_date_on, QString const &eqslcomments, QByteArray const &myadif2)
{
  QString date = QSO_date_on.toString("yyyyMMdd");
  m_qsoLogged = true;
  m_logBook.addAsWorked(call, m_config.bands()->find(dial_freq), mode, date, grid, name);
  QString operator_call = m_config.my_callsign();
  QString my_call = m_config.my_callsign();
  ...
  if (m_config.enable_udp1_adif_sending())
    m_messageClient->logged_ADIF(myadif2);
  ...
  if (m_config.clear_DX() && !logClearDXTimer.isActive() && !m_autoTx && !m_autoseq)
    logClearDXTimer.start((qAbs(int(m_TRperiod) - m_nseq)) * 1000);
  countQSOs();
  ...
  if (ui->cbClearCallsign->isChecked())
  {
    int wcallidx = -1;
    if (m_mode.startsWith("FT"))
      wcallidx = m_wantedCallList.indexOf(call);
    else
      wcallidx = m_wantedCallList.indexOf(Radio::base_callsign(call));
    if (wcallidx >= 0)
    {
      m_wantedCallList.removeAt(wcallidx);
      ui->wantedCall->setText(m_wantedCallList.join(","));
    }
  }
  ...
  if (ui->cbClearGrid->isChecked() && grid.trimmed().length() == 4)
  {
    ...
    if (wgrididx >= 0)
    {
      m_wantedGridList.removeAt(wgrididx);
      ui->wantedGrid->setText(m_wantedGridList.join(","));
    }
  }
  if (m_houndMode && !m_hisCall.isEmpty())
  {
    clearDX(" cleared: QSO logged in DXpedition mode");
    ui->dxCallEntry->setStyleSheet(QString("QLineEdit {color: %1; background: %2}").arg(Radio::convert_dark("#000000", m_useDarkStyle), Radio::convert_dark("#ffffff", m_useDarkStyle)));
  }
}
...
void MainWindow::on_actionFT8_triggered()
{
  ...
  ui->candListSpinBox->setEnabled(true);
  ui->DTCenterSpinBox->setEnabled(true);
  ui->DTCenterSpinBox->setVisible(true);
...
void MainWindow::on_actionWSPR_2_triggered()
{
  ...
  m_TRperiod = 120.0;
  if (m_tci)
    Q_EMIT m_config.transceiver_period(m_TRperiod); // TODO - not thread safe
  else
  {
    m_modulator->setPeriod(m_TRperiod);
    m_detector->setPeriod(m_TRperiod);
  ...
  m_FFTSize = m_nsps / 2;
  if (m_tci)
    Q_EMIT m_config.transceiver_blocksize(m_FFTSize);
  else
  ...
  ui->candListSpinBox->setEnabled(false);
  ui->DTCenterSpinBox->setEnabled(false);
  ui->DTCenterSpinBox->setVisible(false);
  setClockStyle(true);
  progressBar->setMaximum(int(m_TRperiod));
...
void MainWindow::switch_mode(Mode mode)
{
  ...
  {
    if (m_lastMode == "FT4")
      Q_EMIT m_config.transceiver_ft4_mode(false);
    else if (m_mode == "FT4")
      Q_EMIT m_config.transceiver_ft4_mode(true);
    m_qsoHistory.init();
    m_lastMode = m_mode;
    if (m_config.write_decoded_debug())
      writeToALLTXT("QSO history initialized by switch_mode");
  }
  m_okToPost = false;
...
void MainWindow::commonActions()
{
  ...
  if (m_mode == "FT4")
  {
    ...
    {
      m_savedRRR = m_rrr;
      ui->rrrCheckBox->click();
    }
    ui->rrrCheckBox->setEnabled(false);
    ui->rrr1CheckBox->setEnabled(false);
    if (!m_hint)
      ui->hintButton->click();
    ui->hintButton->setEnabled(false);
  }
  ...
  {
    ui->rrrCheckBox->setEnabled(true);
    ui->rrr1CheckBox->setEnabled(true);
    if (m_savedRRR)
      ui->rrrCheckBox->click();
    if (m_mode != "FT8")
...
void MainWindow::WSPR_config(bool b)
{
  ...
  ui->bypassButton->setEnabled(!b);
  ui->singleQSOButton->setEnabled(!b);
  ui->AnsB4Button->setEnabled(!b);
  ui->syncButton->setEnabled(!b);
  ui->syncButton->setVisible(!b);
  if (b)
  {
...
void MainWindow::on_TxFreqSpinBox_valueChanged(int n)
{
  ...
  if (m_tci)
    Q_EMIT m_config.transceiver_trfrequency(n - m_XIT);
  else
...
void MainWindow::on_actionDecFT8cycles1_triggered()
{
  m_nFT8Cycles = 1;
  ui->actionDecFT8cycles1->setChecked(true);
}
void MainWindow::on_actionDecFT8cycles2_triggered()
{
  m_nFT8Cycles = 2;
  ui->actionDecFT8cycles2->setChecked(true);
}
void MainWindow::on_actionDecFT8cycles3_triggered()
{
  m_nFT8Cycles = 3;
  ui->actionDecFT8cycles3->setChecked(true);
}

void MainWindow::on_actionDecFT8SWLcycles1_triggered()
{
  m_nFT8SWLCycles = 1;
  ui->actionDecFT8SWLcycles1->setChecked(true);
}
void MainWindow::on_actionDecFT8SWLcycles2_triggered()
{
  m_nFT8SWLCycles = 2;
  ui->actionDecFT8SWLcycles2->setChecked(true);
}
void MainWindow::on_actionDecFT8SWLcycles3_triggered()
{
  m_nFT8SWLCycles = 3;
  ui->actionDecFT8SWLcycles3->setChecked(true);
}

void MainWindow::on_actionRXfLow_triggered()
{
  m_nFT8RXfSens = 1;
  ui->actionRXfLow->setChecked(true);
}
void MainWindow::on_actionRXfMedium_triggered()
{
  m_nFT8RXfSens = 2;
  ui->actionRXfMedium->setChecked(true);
}
void MainWindow::on_actionRXfHigh_triggered()
{
  m_nFT8RXfSens = 3;
  ui->actionRXfHigh->setChecked(true);
}

void MainWindow::on_actionFT4fast_triggered()
{
  m_nFT4depth = 1;
  ui->actionFT4fast->setChecked(true);
}
void MainWindow::on_actionFT4medium_triggered()
{
  m_nFT4depth = 2;
  ui->actionFT4medium->setChecked(true);
}
void MainWindow::on_actionFT4deep_triggered()
{
  m_nFT4depth = 3;
  ui->actionFT4deep->setChecked(true);
}
...
bool MainWindow::reportRCVD(QStringList msg)
{
  ...
  if (msg.length() == 4)
  { // three words(max index 3) in the message where "CQ DX" is counted as single word
    if (msg.at(1).contains(m_baseCall) && msg.at(2).contains(Radio::base_callsign(m_hisCall)) && (msg.at(3).contains("-") || msg.at(3).contains("+")))
      rprt_rcvd = true;
  }
  return rprt_rcvd;
}
...
bool MainWindow::rReportRCVD(QStringList msg)
{
  ...
  if (msg.length() == 4)
  { // three words(max index 3) in the message where "CQ DX" is counted as single word
    if (msg.at(1).contains(m_baseCall) && msg.at(2).contains(Radio::base_callsign(m_hisCall)) && (msg.at(3).contains("R-") || msg.at(3).contains("R+")))
      rogerrprt_rcvd = true;
  }
...
void MainWindow::on_bandComboBox_activated(int index)
{
  ...
  if (m_config.write_decoded_debug())
    writeToALLTXT("Band changed from bandComboBox, frequency: " + QString::number(frequency));
...
void MainWindow::band_changed(Frequency f)
{
  ...
  {
    if (!m_mode.startsWith("WSPR"))
    { // band hopping preserves auto Tx
      if (f + m_wideGraph->nStartFreq() > m_freqNominal + ui->TxFreqSpinBox->value() || f + m_wideGraph->nStartFreq() + m_wideGraph->fSpan() <=
    ...
    m_okToPost = false;
    if (!m_transmitting && !m_start2 && m_monitoroff)
    {
      monitor(true);
      m_monitoroff = false;
    }
    ...
    if (ui->outAttenuation->value() == 1 && m_outAttenuation != 1)
    {
    ...
    if (qAbs(fDelta) > 1000000)
    {
      ...
      if (m_config.pwrBandTxMemory() && !m_tune)
      {
        if (m_mode == "JT9+JT65" && m_modeTx == "JT65")
        {
          curBand = m_config.bands()->find(m_freqNominal) + m_modeTx;
        }
        else
        {
          curBand = m_config.bands()->find(m_freqNominal) + m_mode;
        }
        if (m_pwrBandTxMemory.contains(curBand))
        {
          m_PwrBandSetOK = false;
          ui->outAttenuation->setValue(m_pwrBandTxMemory[curBand].toInt());
          m_PwrBandSetOK = true; /* printf("set power from bandchange % lld %lld %s %d\n",m_lastDisplayFreq,m_freqNominal,curBand.toStdString().c_str(),m_pwrBandTxMemory[curBand].toInt());*/
        }
        else
        {
          m_pwrBandTxMemory[curBand] = ui->outAttenuation->value();
...
void MainWindow::enable_DXCC_entity()
{
  if (m_mode.left(4) != "WSPR" && (m_callNotif != m_config.callNotif() || m_callsign != m_config.my_callsign() || m_gridNotif != m_config.gridNotif() || m_grid != m_config.my_grid() || m_timeFrom != m_config.timeFrom() || m_strictdirCQ != m_config.strictdirCQ()))
  {
    if (m_callNotif != m_config.callNotif() || m_callsign != m_config.my_callsign() || m_gridNotif != m_config.gridNotif() || m_grid != m_config.my_grid() || m_timeFrom != m_config.timeFrom())
    {
      m_qsoHistory.init();
      if (m_config.write_decoded_debug())
        writeToALLTXT("QSO history initialized by enable_DXCC_entity");
      m_logBook.init(m_config.callNotif() ? m_config.my_callsign() : "", m_config.gridNotif() ? m_config.my_grid() : "", m_config.timeFrom());
      m_callsign = m_config.my_callsign();
      m_grid = m_config.my_grid();
      m_callNotif = m_config.callNotif();
...
{
  bool isAllowedAuto73 = isAutoSeq73(text);
  if (!m_FTsetAutoSeqOff && !isAutoSeq73(text))
    m_FTsetAutoSeqOff = true;
  if (isAllowedAuto73)
    m_FTsetAutoSeqOff = false;
  if (!text.contains(QRegularExpression{R"([@#&^])"}) && !text.isEmpty())
  {
    ...
    else
    {
      ...
      {
        ui->freeTextMsg->setStyleSheet(QString("background: %1;color: %2").arg(Radio::convert_dark("#ffff00", m_useDarkStyle), Radio::convert_dark("#000000", m_useDarkStyle)));
      }
...
void MainWindow::on_freeTextMsg_currentIndexChanged(int index)
{
  ...
  // it is dedicated to change free message during Tx if new message is selected from the list
  if (m_transmitting && ui->rbFreeText->isChecked() && index != m_oldFreeMsgIndex)
    m_restart = true;
...
void MainWindow::on_tuneButton_clicked(bool checked)
{
  ...
  if (checked && m_tune == false)
  { // we're starting tuning so remember Tx and change pwr to Tune value
  ...
  if (m_tci)
    ...
    Q_EMIT tune(checked);
  if (checked && m_config.tunetimer() && !m_tuneup)
    StopTuneTimer.start(m_config.tunetimer() * 1000); // shall not start at WSPR band hopping
}

void MainWindow::stop_tuning()
{
  if (StopTuneTimer.isActive())
    StopTuneTimer.stop();
...
void MainWindow::on_stopTxButton_clicked() // Stop Tx
{
  ...
  if (m_tune)
    stop_tuning();
  if (m_enableTx and !m_tuneup)
    enableTx_mode(false);
  m_btxok = false;
  m_nlasttx = 0;
  //  if(m_autofilter && m_autoseq && m_filter) autoFilter (false);
  if (m_filter && (m_FilterState == 1 || m_FilterState == 2))
    autoFilter(false);
  if (!m_transmitting && g_iptt == 1)
    m_haltTrans = true;
  if (m_haltTxWritten)
    ...
    writeHaltTxEvent("Halt Tx button clicked ");
  if (m_transmitting && m_skipTx1 && !m_hisCall.isEmpty() && (m_ntx == 2 || m_QSOProgress == REPORT))
    m_qsoHistory.remove(m_hisCall);
...
void MainWindow::setXIT(int n, Frequency base)
{
  ...
  if (!m_bSimplex && base)
  {
    ...
    if ((m_monitoring || m_transmitting) && m_config.is_transceiver_online() && m_config.split_mode())
    {
      ...
      if (base == 1908000)
      {
        if (m_crossbandOptionEnabled && m_m_prefix != "JA")
          m_freqTxNominal -= 68000;
      }
      else if (base == 1810000)
      {
        if (m_crossbandHLOptionEnabled && m_m_prefix != "HL")
          m_freqTxNominal += 30000;
      }
...
void MainWindow::on_skipTx1_clicked(bool checked)
{
  ...
  {
    if (ui->txrb2->isChecked())
      on_txb1_clicked();
    if (ui->genMsg->text() == ui->tx2->text())
      ui->genMsg->setText(ui->tx1->text());
  }
}
...
void MainWindow::handle_transceiver_update(Transceiver::TransceiverState const &s)
{
  ...
  if (!s.ptt() && m_rigState.ptt() && (m_transmitting || m_tune))
    haltTx("Halt Tx from rig detected ");
                                    ...
                                    // (caveat - DX Lab Suite Commander)
  {
    ...
    if (m_tx_when_ready && g_iptt)
    {
      ...
      if (m_config.write_decoded_debug())
        writeToALLTXT("ptt1Timer started");
    }
    m_tx_when_ready = false;
  }
  ...
  if (m_config.do_pwr())
  {
    ...
    if (m_rigState.swr() != s.swr())
    {
      ...
      {
        if (s.swr() < 1000)
          ui->SWRlabel->setText(QString{"swr%1"}.arg(s.swr() / 100., 0, 'f', 2));
        else
          ui->SWRlabel->setText(QString{"swr%1"}.arg(s.swr() / 100., 0, 'f', 1));
      }
      else
        ui->SWRlabel->setText("");
    }
  ...
  if (old_state.online() == false && s.online() == true)
  {
    on_monitorButton_clicked(true);
    if (m_config.write_decoded_debug())
      writeToALLTXT("handle_transceiver_update: transceiver state transition from offline to online");
    if (m_mode == "FT8")
      on_actionFT8_triggered();
    else if (m_mode == "FT4")
      on_actionFT4_triggered();
  ...
  if (s.frequency() != old_state.frequency() || s.split() != m_splitMode)
  {
    ...
    if (!s.ptt())
    {
      ...
      if (m_lastDialFreq != m_freqNominal)
      {
        ...
        {
          JTDXMessageBox::warning_message(this, "", tr("File Open Error"), tr("Cannot open \"%1\" for append: %2").arg(f2.fileName()).arg(f2.errorString()));
        }
        //              }
...
void MainWindow::transmit(double snr)
{
  ...
  else if (m_modeTx == "FT4")
  {
    ...
    if (m_tci)
      Q_EMIT m_config.transceiver_modulator_start(NUM_FT4_SYMBOLS, 576.0, ui->TxFreqSpinBox->value() - m_XIT, toneSpacing, true, snr, m_TRperiod);
    else
  ...
  else if (m_modeTx == "JT65")
  {
    ...
    if (m_tci)
      Q_EMIT m_config.transceiver_modulator_start(NUM_JT65_SYMBOLS, 4096.0 * 12000.0 / 11025.0, ui->TxFreqSpinBox->value() - m_XIT, toneSpacing, true, snr, m_TRperiod);
    else
  ...
  {
    double sps = m_nsps;
    m_toneSpacing = 12000.0 / 6912.0;
    if (m_tci)
      Q_EMIT m_config.transceiver_modulator_start(NUM_JT9_SYMBOLS, sps, ui->TxFreqSpinBox->value() - m_XIT, m_toneSpacing, true, snr, m_TRperiod);
    else
  ...
  {
    double sps = m_nsps;
    m_toneSpacing = 4.0 * 12000.0 / 6912.0;
    if (m_tci)
      Q_EMIT m_config.transceiver_modulator_start(NUM_T10_SYMBOLS, sps, ui->TxFreqSpinBox->value() - m_XIT, m_toneSpacing, true, snr, m_TRperiod);
    else
  ...
  {
    if (m_tci)
      Q_EMIT m_config.transceiver_modulator_start(NUM_WSPR_SYMBOLS, 8192.0, ui->TxFreqSpinBox->value() - 1.5 * 12000 / 8192, m_toneSpacing, true, snr, m_TRperiod);
    else
      Q_EMIT sendMessage(NUM_WSPR_SYMBOLS, 8192.0, ui->TxFreqSpinBox->value() - 1.5 * 12000 / 8192, m_toneSpacing, m_soundOutput,
                         m_config.audio_output_channel(), true, snr, m_TRperiod);
  }
  if (m_nlasttx == 0 && !m_tune)
    m_nlasttx = m_ntx;
}
...
{
  if (!m_prefixes)
    m_prefixes.reset(new HelpTextWindow{tr("Prefixes"), R"(Short-list of Add-On DXCC Prefixes:
...
bool MainWindow::isAutoSeq73(QString const &text)
{
  ...
  bool b = ((partsSize > 0 && (parts[0] == "73" || parts[0] == "TNX" || parts[0] == "TKS" || parts[0] == "TU")) || (partsSize > 1 && (parts[1] == "73" || parts[1] == "TNX" || parts[1] == "TKS" || parts[1] == "TU")) || (partsSize > 2 && (parts[2] == "73" || parts[2] == "TNX" || parts[2] == "TKS" || parts[2] == "TU")) || (partsSize > 3 && (parts[3] == "73" || parts[3] == "TNX" || parts[3] == "TKS" || parts[3] == "TU")));
  return b;
}
...
void MainWindow::enableHoundAccess(bool b)
{
  if (b)
  {
    ui->actionEnable_hound_mode->setEnabled(true);
    ui->HoundButton->setEnabled(true);
  }
  else
  {
    ui->actionEnable_hound_mode->setChecked(false);
...
void MainWindow::setHoundAppearance(bool hound)
{
  bool b = !hound;
  ui->tx2->setEnabled(b);
  ui->txb2->setEnabled(b);
  ui->tx4->setEnabled(b);
  ui->txb4->setEnabled(b);
  ui->tx5->setEnabled(b);
  ui->txb5->setEnabled(b);
  ui->tx6->setEnabled(b);
  ui->txb6->setEnabled(b);
  ui->pbCallCQ->setEnabled(b);
  ui->pbAnswerCaller->setEnabled(b);
  ui->pbSendRRR->setEnabled(b);
  ui->pbSend73->setEnabled(b);
  ui->freeTextMsg->setEnabled(b);
  ui->rbFreeText->setEnabled(b);
}

void MainWindow::setLastLogdLabel()
{
  if (m_lastloggedcall.isEmpty())
  {
    lastlogged_label->setText(tr("Logd "));
    lastlogged_label->setStyleSheet(QString("QLabel{background: %1}").arg(Radio::convert_dark("#ffffff", m_useDarkStyle)));
  }
  else
  {
    lastlogged_label->setText(tr("Logd ") + m_lastloggedcall);
    lastlogged_label->setStyleSheet(QString("QLabel{background: %1}").arg(Radio::convert_dark("#7fff7f", m_useDarkStyle)));
  }
}
...
void MainWindow::writeToALLTXT(QString const &text)
{
  ...
  if (f.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Append))
  {
    ...
    if (text.endsWith("count reached"))
      out << "Counters: "
...
void MainWindow::setTxMsgBtnColor()
{
  ...
  {
    if (m_ntx == 1)
      ui->txb1->setStyleSheet(QString("QPushButton{background: %1}").arg(Radio::convert_dark("#88ff88", m_useDarkStyle)));
    else if (m_ntx == 2)
      ui->txb2->setStyleSheet(QString("QPushButton{background: %1}").arg(Radio::convert_dark("#88ff88", m_useDarkStyle)));
    else if (m_ntx == 3)
      ui->txb3->setStyleSheet(QString("QPushButton{background: %1}").arg(Radio::convert_dark("#88ff88", m_useDarkStyle)));
    else if (m_ntx == 4)
      ui->txb4->setStyleSheet(QString("QPushButton{background: %1}").arg(Radio::convert_dark("#88ff88", m_useDarkStyle)));
    else if (m_ntx == 5)
      ui->txb5->setStyleSheet(QString("QPushButton{background: %1}").arg(Radio::convert_dark("#88ff88", m_useDarkStyle)));
    else if (m_ntx == 6)
      ui->txb6->setStyleSheet(QString("QPushButton{background: %1}").arg(Radio::convert_dark("#88ff88", m_useDarkStyle)));
  }
  else if (1 == ui->tabWidget->currentIndex())
  {
    if (m_QSOProgress == CALLING)
      ui->pbCallCQ->setStyleSheet(QString("QPushButton{background: %1}").arg(Radio::convert_dark("#88ff88", m_useDarkStyle)));
    else if (m_QSOProgress == REPLYING)
      ui->pbAnswerCQ->setStyleSheet(QString("QPushButton{background: %1}").arg(Radio::convert_dark("#88ff88", m_useDarkStyle)));
    else if (m_QSOProgress == REPORT)
      ui->pbAnswerCaller->setStyleSheet(QString("QPushButton{background: %1}").arg(Radio::convert_dark("#88ff88", m_useDarkStyle)));
    else if (m_QSOProgress == ROGER_REPORT)
      ui->pbSendReport->setStyleSheet(QString("QPushButton{background: %1}").arg(Radio::convert_dark("#88ff88", m_useDarkStyle)));
    else if (m_QSOProgress == ROGERS)
      ui->pbSendRRR->setStyleSheet(QString("QPushButton{background: %1}").arg(Radio::convert_dark("#88ff88", m_useDarkStyle)));
    else if (m_QSOProgress == SIGNOFF)
      ui->pbSend73->setStyleSheet(QString("QPushButton{background: %1}").arg(Radio::convert_dark("#88ff88", m_useDarkStyle)));
  }
}
...
void MainWindow::resetTxMsgBtnColor()
{
  if (0 == ui->tabWidget->currentIndex())
  {
    ui->txb1->setStyleSheet("");
    ui->txb2->setStyleSheet("");
    ui->txb3->setStyleSheet("");
    ui->txb4->setStyleSheet("");
    ui->txb5->setStyleSheet("");
    ui->txb6->setStyleSheet("");
  }
  else if (1 == ui->tabWidget->currentIndex())
  {
    ui->pbCallCQ->setStyleSheet("");
    ui->pbAnswerCQ->setStyleSheet("");
    ui->pbAnswerCaller->setStyleSheet("");
    ui->pbSendReport->setStyleSheet("");
    ui->pbSendRRR->setStyleSheet("");
    ui->pbSend73->setStyleSheet("");
  }
}
...
void MainWindow::pskSetLocal()
{
  ...
  auto matches = stations->match(stations->index(0, StationList::band_column), Qt::DisplayRole, ui->bandComboBox->currentText(), 1, Qt::MatchExactly);
  QString antenna_description;
  if (!matches.isEmpty())
    antenna_description = stations->index(matches.first().row(), StationList::description_column).data().toString();
...
void MainWindow::replyToUDP(QTime time, qint32 snr, float delta_time, quint32 delta_frequency, QString const &mode, QString const &message_text, bool /*low_confidence*/, quint8 /*modifiers*/)
{
  if (m_config.write_decoded_debug())
    writeToALLTXT("UDP Reply request received: " + message_text);
  if (!m_config.accept_udp_requests())
  {
    if (m_config.write_decoded_debug())
      writeToALLTXT("UDP Reply request ignored: UDP accept option disabled");
    return;
  }
  ...
  if (m_acceptUDP == 1)
  {
    acceptMsg = message_text.contains(QRegularExpression{R"(^(CQ |CQDX |QRZ))"});
  }
  else if (m_acceptUDP == 2)
  {
    acceptMsg = message_text.contains(QRegularExpression{R"(^(CQ |CQDX |QRZ)|(.+ 73|.+ RR73))"});
  }
  else if (m_acceptUDP == 3)
  {
    acceptMsg = true;
  ...
  if (acceptMsg)
  {
    ...
    int position;
    if (m_mode.startsWith("FT"))
      position = messages.lastIndexOf(msgText.left(42));
    else
      position = messages.lastIndexOf(msgText.left(40));
    if (position < 0)
    {
    ...
    if (position >= 0)
    {
      ...
      txwatchdog(false);
      if (m_windowPopup)
        QApplication::alert(this); // raise up task bar under window popup option, Logger32 compatibility
      m_decodedText2 = false;
      if (m_config.write_decoded_debug())
        writeToALLTXT("UDP Reply request processed");
    }
    else
    {
      //          qDebug () << "reply to message request ignored, decode not found:" << msgText;
      if (m_config.write_decoded_debug())
        writeToALLTXT("UDP Reply request ignored, decode not found: " + msgText);
    }
  }
  else
  {
    ...
    if (m_config.write_decoded_debug())
      writeToALLTXT("rejecting UDP request to reply as decode is not valid: " + message_text);
  }
}
...
void MainWindow::postDecode(bool is_new, QString const &message)
{
  ...
  if (parts.size() >= 5)
  {
    ...
    m_messageClient->decode(is_new, QTime::fromString(parts[0], has_seconds ? "hhmmss" : "hhmm"), parts[1].toInt(), parts[2].toFloat(), parts[3].toUInt(), parts[4], decode.mid(has_seconds ? 23 : 21, 23), low_confidence, m_diskData);
  }
}
...
void MainWindow::postWSPRDecode(bool is_new, QStringList parts)
{
  ...
  m_messageClient->WSPR_decode(is_new, QTime::fromString(parts[0], "hhmm"), parts[1].toInt(), parts[2].toFloat(), Radio::frequency(parts[3].toFloat(), 6), parts[4].toInt(), parts[5], parts[6], parts[7].toInt(), m_diskData);
}

void MainWindow::networkError(QString const &e)
{
  if (JTDXMessageBox::Retry == JTDXMessageBox::warning_message(this, "", tr("Network Error"), tr("Error: %1\nUDP server %2:%3").arg(e).arg(m_config.udp_server_name()).arg(m_config.udp_server_port()), "", JTDXMessageBox::Cancel | JTDXMessageBox::Retry, JTDXMessageBox::Cancel))
  {
    // retry server lookup
...
void MainWindow::p1ReadFromStdout() // p1readFromStdout
{
  while (p1.canReadLine())
  {
    ...
    {
      if (t.contains(" " + m_config.my_callsign() + " "))
        continue;
      if (t.contains(" <" + m_config.my_callsign() + "> "))
        continue;
    }
    ...
    if (t.indexOf("<DecodeFinished>") >= 0)
    {
      ...
      m_nWSPRdecodes = 0;
      ui->DecodeButton->setChecked(false);
      if (m_uploadSpots && m_config.is_transceiver_online())
      { // need working rig control
#if QT_VERSION >= QT_VERSION_CHECK(5, 15, 0)
...
void MainWindow::WSPR_history(Frequency dialFreq, int ndecodes)
{
  ...
  {
    JTDXMessageBox::warning_message(this, "", tr("File Error"), tr("Cannot open \"%1\" for append: %2").arg(f.fileName()).arg(f.errorString()));
  }
}
...
void MainWindow::uploadSpots()
{
  ...
  if (m_diskData || !m_config.is_transceiver_online())
    return;
  if (m_uploading)
  {
...
void MainWindow::on_cbUploadWSPR_Spots_toggled(bool b)
{
  m_uploadSpots = b;
  if (m_uploadSpots)
    ui->cbUploadWSPR_Spots->setStyleSheet("");
  if (!m_uploadSpots)
    ui->cbUploadWSPR_Spots->setStyleSheet(QString("QCheckBox{background: %1}").arg(Radio::convert_dark("#ffff00", m_useDarkStyle)));
}
...
void MainWindow::WSPR_scheduling()
{
      ...
      && !m_config.is_dummy_rig() && ui->band_hopping_group_box->isChecked())
  {
    ...
    if (hop_data.frequencies_index_ >= 0)
    { // new band
...
void MainWindow::on_the_minute()
{
  ...
  if (m_config.watchdog() && !m_mode.startsWith("WSPR"))
  {
    ...
    if (update && (m_idleMinutes < m_config.watchdog()))
    {
      ++m_idleMinutes;
      update_watchdog_label();
    }
  ...
  if (!m_transmitting && m_mode == "FT8" && (m_jtdxtime->currentMSecsSinceEpoch2() - m_mslastTX) > 120000)
    m_lapmyc = 0;
}
...
{
  m_wantedchkd = b;
  ui->labWantCall->setVisible(b);
  ui->wantedCall->setVisible(b);
  ui->labWantCountry->setVisible(b);
  ui->wantedCountry->setVisible(b);
  ui->labWantPfx->setVisible(b);
  ui->wantedPrefix->setVisible(b);
  ui->labWantGrid->setVisible(b);
  ui->wantedGrid->setVisible(b);
  ui->cbClearCallsign->setVisible(b);
  ui->cbClearGrid->setVisible(b);
  dynamicButtonsInit();
}

void MainWindow::on_cbShowSpot_toggled(bool b)
{
  ui->spotMsgLabel->setVisible(b);
  ui->spotEditLabel->setVisible(b);
  ui->spotLineEdit->setVisible(b);
  ui->propEditLabel->setVisible(b);
  ui->propLineEdit->setVisible(b);
}
...
void MainWindow::dynamicButtonsInit()
{
  ...
  if (m_menus)
  {
    if (!m_wantedchkd)
    {
      if (height <= 435)
      {
        ui->bypassButton->hide();
        ui->singleQSOButton->hide();
        ui->AnsB4Button->hide();
        ui->stopButton->hide();
      }
      else if (height > 435 && height <= 475)
      {
        ui->bypassButton->show();
        ui->singleQSOButton->hide();
        ui->AnsB4Button->hide();
        ui->stopButton->hide();
      }
      ...
      {
        ui->bypassButton->show();
        ui->singleQSOButton->show();
        ui->AnsB4Button->hide();
        ui->stopButton->hide();
      }
      else if (height > 500 && height <= 525)
      {
        ui->bypassButton->show();
        ui->singleQSOButton->show();
        ui->AnsB4Button->show();
